...
def build_spark_submit_cmd(config, benchmark, args):
    ...
    # Pass profiling dirs and upload path through to tpcbench.py
    if args.jfr:
        cmd += ["--jfr-dir", args.jfr_dir]
    if args.async_profiler:
        cmd += ["--async-profiler-dir", args.async_profiler_dir]
    if args.profile_upload:
        cmd += ["--profile-upload", args.profile_upload]
...
def main():
    ...
    parser.add_argument(
        "--profile-upload",
        help="Remote path (e.g. s3a://bucket/profiles) to upload profiling artifacts",
    )
    ...
    # Create profiling output directories (skip for dry-run and K8s mode
    # where dirs are inside the pod, not on the submitter)
    if not args.dry_run and not is_k8s_master():
        if args.jfr:
