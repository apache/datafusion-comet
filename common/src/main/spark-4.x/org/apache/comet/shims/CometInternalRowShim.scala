...
/**
 * Throwing-default implementations for `SpecializedGetters` methods that were added in Spark 4.x:
 ...
 * these defaults the compiled class fails its abstract-method check at class-load time. The
 * spark-3.x profile ships an empty shim because none of these getters exist there.
 */
