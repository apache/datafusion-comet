...
- Known divergences and gaps:
  ...
  - `CAST(<float|double> AS DECIMAL)` rounding may differ from Spark (`Incompatible`, gated by `spark.comet.expression.Cast.allowIncompatible`, tracked at [#1371](https://github.com/apache/datafusion-comet/issues/1371)).
...
- Performance (tuned 2026-07-29, PR [#5130](https://github.com/apache/datafusion-comet/pull/5130)): string-to-timestamp and string-to-timestamp_ntz (`timestamp_parser` / `timestamp_ntz_parser`) classify the input shape once with a single ASCII byte scan, replacing up to 28 sequential regex `is_match` calls per row (21 for the NTZ variant, which matched three separate times). Non-ASCII input keeps regex semantics, since `\d` is Unicode-aware, but via one `RegexSet` pass. 32-82% faster (up to 5.6x) with every shape improved. Benchmark: `benches/cast_string_to_timestamp.rs`.
