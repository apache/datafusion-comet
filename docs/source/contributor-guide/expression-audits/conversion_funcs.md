...
- Known divergences and gaps:
  - `CAST(<binary> AS STRING)` decodes the bytes with `decode_utf8_spark_lossy`, replacing invalid UTF-8 with `U+FFFD` exactly as the JVM's `new String(bytes, UTF_8)` does (the same decoder as the native shuffle). It matches Spark's rendered output, but decoding is not byte-preserving, so it diverges from Spark for operations that read the underlying bytes: round-trips such as `CAST(CAST(x AS STRING) AS BINARY)`, and value identity, since distinct ill-formed byte sequences all decode to `U+FFFD` and can therefore compare equal in Comet where Spark compares the raw bytes ([#4764](https://github.com/apache/datafusion-comet/issues/4764)).
  - Spark 4.0 collated `StringType` is not explicitly guarded; pattern equality is expected to keep collated-string casts falling back, but there is no test ([#4489](https://github.com/apache/datafusion-comet/issues/4489); umbrella [#2190](https://github.com/apache/datafusion-comet/issues/2190)).
  - Spark 4.1 `TimeType` casts have no explicit `Unsupported` arm; they fall back implicitly but do not appear in the auto-generated compatibility doc ([#4490](https://github.com/apache/datafusion-comet/issues/4490)).
...
- Performance (tuned 2026-08-27, PR [#5457](https://github.com/apache/datafusion-comet/pull/5457)): nullable nested casts can skip the preparation copy for signed integer widening and unchanged fixed-width leaves, including struct trees. Lists/maps with unused backing values or dense null rows still compact. The new reuse path also does not bypass compaction for nested variable-length containers or unchanged string/binary children. Against the unmodified PR head `8b072abb`, the measured nullable `Int32` to `Int64` struct and sparse list/map shapes used 12-92% less batch time. Criterion covered 46 shapes with 8,192 parent rows, list/map widths 4 and 64, 0-99% parent nulls, slices, a large hidden list, and checked DATE controls. Six initial 1.8-3.3% control regressions did not reproduce when repeated in the opposite binary order; no reproducible regression was found among these measured shapes. Benchmark: `benches/cast_nested.rs`.
