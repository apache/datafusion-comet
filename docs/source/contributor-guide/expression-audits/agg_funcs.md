...
- Buffer shape (applies equally to `collect_set`): both are `TypedImperativeAggregate`s, so Spark's `aggBufferAttributes` declares the intermediate buffer as `BinaryType` (serialized state) while the native accumulator's `state_fields` is a `List`. `CometBaseAggregate.adjustOutputForNativeState` rewrites Comet `Partial` and `PartialMerge` outputs to the list shape. When a Spark `Partial` feeds a Comet `PartialMerge`, Comet validates and decodes the single-field `UnsafeRow`/`UnsafeArrayData` binary buffer before merging it ([#4724](https://github.com/apache/datafusion-comet/issues/4724)).
