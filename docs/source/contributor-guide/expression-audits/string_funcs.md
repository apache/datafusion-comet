...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened to `StringTypeWithCollation(supportsTrimCollation = true)`; behaviour unchanged for `UTF8_BINARY`. Comet does not propagate collation, so non-default collations may diverge silently ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened to `StringTypeWithCollation(supportsTrimCollation = true)`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): `StringTrim` (the rewrite target) routes through `CollationSupport.StringTrim.exec` and uses `StringTypeNonCSAICollation(supportsTrimCollation = true)`; semantics unchanged for `UTF8_BINARY`. Non-default collations may diverge in Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.Contains.exec(..., collationId)`; behaviour identical for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Known limitation: because there is no `CometExpressionSerde[StringDecode]` registration, `decode` does not surface in the auto-generated compatibility docs ([#4466](https://github.com/apache/datafusion-comet/issues/4466)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.EndsWith.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).

## initcap

- Spark 3.4.3 (audited 2026-05-27): identical to 3.5.8.
- Spark 3.5.8 (audited 2026-05-27): baseline. `string.toLowerCase.toTitleCase` on `UTF8String`; word boundary is Java `Character.isWhitespace`. DataFusion `initcap` splits on `!is_alphanumeric()` (hyphens, apostrophes, and punctuation all split words), so `CometInitCap` reports `Compatible` with a `NativeOptIn` and dispatches to Spark's generated code by default. The native path is used only when `spark.comet.expression.InitCap.allowIncompatible=true`.
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.InitCap.exec` (collation- and ICU-aware) and propagates `child.dataType`. Comet ignores collation; 3.x divergences persist plus collation/ICU mismatches ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StringInstr.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened with `StringTypeWithCollation`; behaviour unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Native candidate (assessed 2026-08-13): Deferred ([#5353](https://github.com/apache/datafusion-comet/issues/5353)). Compatibility: Low for the general case (three distinct non-ASCII behaviours across the pinned versions: JVM default locale on 3.4.3 and 3.5.9, ICU root locale on 4.0.4/4.1.3/4.2.0 where `spark.sql.icu.caseMappings.enabled` defaults to true, and back to JVM default locale when that config is off; the JDK branch reads the host JVM locale, which the serialised plan does not capture). Upside: High but not collectable (2 allocations per non-null row unconditionally on the ASCII path, `toUpperCaseAscii` = `convertAscii` allocating a fresh `byte[]` plus a `UTF8String` wrapper, about 16k per 8192-row batch, measured from the emitted kernel; `upper`/`lower` are among the most common string expressions and `ilike` rewrites to `Lower`). Blocker: the only provably-agreeing subset is all-ASCII input, which Spark itself branches on via `UTF8String.isFullAscii()`, but that is a per-batch runtime property and Comet fixes the native-versus-dispatcher choice at plan time in serde with no way for a kernel to defer a batch. Unblocked by a per-batch defer-to-dispatcher mechanism (adjacent to [#4825](https://github.com/apache/datafusion-comet/issues/4825) but per batch), or by a Unicode-version-pinned proof that Rust case mapping matches ICU root locale. The same blocker gates `initcap` and the JSON family.
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StringTrimLeft.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened to `StringTypeWithCollation`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): adds `nullIntolerant: Boolean` field; `dataType` becomes `str.dataType` (collation-tracking). Semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Native candidate (assessed 2026-08-13): Recommended for the non-empty-literal search subset ([#5354](https://github.com/apache/datafusion-comet/issues/5354)). Compatibility: High (the entire gap is one input condition, an empty search string, which Spark short-circuits and DataFusion does not; identical semantics across 3.4.3, 3.5.9, 4.0.4, 4.1.3, and 4.2.0 for `UTF8_BINARY`, where Spark 4.x's `CollationSupport.StringReplace.execBinary` is the same `src.replace(search, replace)` call as 3.x; the guard is an expression property, not a data property, so unlike [#5353](https://github.com/apache/datafusion-comet/issues/5353) it needs no per-batch fallback). Upside: Medium to High, measured (80 bytes/row at 0% match density rising to 136 at 100%, via `getThreadAllocatedBytes` over off-heap-backed inputs including the kernel's output-write branch, roughly 650KB to 1.1MB of heap churn per 8192-row batch). Note the allocation is unconditional: `UTF8String.replace` returns the input unchanged on a non-match, but that input is `fromAddress`-backed, so the kernel's output write takes the `getBytes()` copy branch anyway. Time is flat at 170-195 ns/row and scan-bound, so the wall-clock win is unproven and left to the implementing PR to measure. No native code needed: the kernel exists and this is a `getSupportLevel` change.
...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened with collation; uses `UnaryMinus(len, failOnError = false)` to avoid integer-overflow exceptions on `len = Int.MinValue`. Semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StringTrimRight.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): wraps the regex via `CollationSupport.collationAwareRegex` and changes `dataType` to `ArrayType(str.dataType, ...)`. Comet does not honour collation flags ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).

## startswith

- Spark 3.4.3 (audited 2026-05-27): identical to 3.5.8.
- Spark 3.5.8 (audited 2026-05-27): baseline. `UTF8String.startsWith` on `StringType`; binary form routed to `BinaryPredicate`.
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StartsWith.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): `inputTypes` widened with `StringTypeWithCollation`; semantics unchanged for `UTF8_BINARY`. `SparkSubstring` implements Spark's 1-indexed, negative-start, and negative-length clamping. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.SubstringIndex.exec` and propagates `strExpr.dataType`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StringTranslate.exec`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
...
- Spark 4.0.1 (audited 2026-05-27): routes through `CollationSupport.StringTrim.exec` and uses `StringTypeNonCSAICollation`; semantics unchanged for `UTF8_BINARY`. Non-default collations not honoured by Comet ([#4496](https://github.com/apache/datafusion-comet/issues/4496)).
