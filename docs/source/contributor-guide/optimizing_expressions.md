...
| **Read from the offset buffer directly**                           | `list_array.value(i)` allocating a sliced `ArrayRef` per row                                          | `spark_size` scalar path: `value_length` instead of materializing a sliced array; Map array path (~6-13x): `MapArray::offsets().windows(2)` diff, `length` kernel doesn't support Map; LargeList array path (~6.6x): `offsets().windows(2)` diff into Int32, skips `length`'s Int64→Int32 cast |
| **Reuse Arrow `length` kernel + cheap null rewrite**               | Per-row `value_length` / builder loop, or `zip` via `MutableArrayData`                                | `spark_size` (~12x on no-null List): `length` for List/FixedSizeList, then `into_parts` + `set_indices` to patch null slots to `-1`                                                   |
