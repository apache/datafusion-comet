...
run against Spark 3.5.9; Iceberg 1.11.0 runs against Spark 4.1.3. All four versions run in the
merge queue, or earlier on a pull request labeled `run-iceberg-tests`; none runs on an unlabeled
pull request. All caller
...
own Spark tests, so without the label the first Iceberg verdict is the merge queue's.
