...
- A location's store is an `AmazonS3` whose bridge is bound to the location itself, so the vendor sees one stable path per location. It is built on first use, usually inside an async read, from an `S3StoreTemplate` that resolved the region when the bucket's store was created, so building never blocks on the Tokio runtime.
