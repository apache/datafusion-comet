...
STS endpoint selection (`configure_sts_endpoint`) mirrors reqsign, with FIPS taking strict precedence. FIPS (`AWS_USE_FIPS_ENDPOINT` / profile) always uses the SDK's regional FIPS endpoint, because there is no global FIPS STS endpoint; an accompanying `AWS_STS_REGIONAL_ENDPOINTS=legacy` is ignored with a one-time warning. Otherwise `regional` keeps the SDK's regional resolution, and `legacy` or unset overrides the client to the global `sts.amazonaws.com` endpoint signed as the partition's global region (`us-east-1`, or `cn-north-1` for China; GovCloud/ISO have no global endpoint and fall back to regional). This preserves reqsign's endpoint choice so an executor whose egress only reaches the global endpoint keeps working after the take-over.
