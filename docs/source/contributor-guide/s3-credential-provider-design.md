...
It exists because the implicit IRSA credential path mishandles STS throttling. `AssumeRoleWithWebIdentity` is resolved per reader thread; a concurrent startup burst throttles STS. On the Iceberg path opendal's default reqsign chain does not retry the throttle and falls through to the EKS node instance role, which lacks bucket access, turning a transient throttle into a hard `403` (this is the reported failure). On the Parquet path the AWS SDK default chain does retry a little and stops on a provider error rather than downgrading, but it still gives every reader its own assume-role call with no shared cache. When both `AWS_WEB_IDENTITY_TOKEN_FILE` and `AWS_ROLE_ARN` are set and no explicit credentials are configured, both scan paths install `WebIdentityCredentialProvider` instead of the default chain (Parquet: the `None` branch of `s3.rs::create_store`; Iceberg: `iceberg_common.rs::build_s3_credential_loader` returns a `CustomAwsCredentialLoader` instead of `Ok(None)`). It:
...
- is web-identity ONLY, with no IMDS/instance-role fallback, so a throttle that outlasts the retries errors instead of downgrading,
- resolves region, `use_fips` and `use_dual_stack` the same way the default chain would (environment then profile) and forwards them to the STS client, so `AWS_USE_FIPS_ENDPOINT` / `AWS_USE_DUALSTACK_ENDPOINT` are honored rather than defaulted off, and
- caches one credential per process keyed by identity `(role_arn, token_file, region)` plus the resolved retry/refresh settings (so a catalog's own knobs are honored regardless of init order), single-flights refreshes so a reader burst triggers one STS call, briefly remembers a failed refresh so a throttled burst also costs one call rather than one per reader, and jitters the refresh deadline per process so cluster-wide refreshes desync.

Unlike the bridge, this provider owns the whole credential lifecycle (there is no vendor to delegate to), which is why caching lives here. The knobs are read from the config bag with sensible defaults; see the user guide "EKS / IRSA" section. It stands aside for any credential source the default chain ranks ahead of web-identity -- a bridge class, `fs.s3a.aws.credentials.provider`, catalog static keys / `client.assume-role.arn`, static `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY`, or a configured profile (`AWS_PROFILE` or a shared credentials file) -- so it only changes the otherwise-default behavior.
