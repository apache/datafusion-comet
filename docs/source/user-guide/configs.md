...
| spark.comet.shuffle.sizeInBytesMultiplier | Comet produces smaller shuffle files due to columnar compression and this can result in Spark choosing a different join strategy due to the estimated size of the exchange being smaller. Comet will multiple sizeInBytes by this amount to avoid regressions in join strategy. | 1.0 |
