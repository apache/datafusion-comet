...
<!-- This table is hand-maintained (the doc build cannot see the contrib's config provider) and
guarded by DeltaScanContribSuite, which checks every DeltaScanConf entry's key, doc, and default. -->
