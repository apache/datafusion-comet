...
| `spark.comet.parquet.write.enabled` | Whether to enable native Parquet write through Comet. When enabled, Comet will intercept Parquet write operations and execute them natively. Supports all data types including complex types (arrays, maps, structs). This feature is experimental and does not yet support partitioned or bucketed writes. | false |
