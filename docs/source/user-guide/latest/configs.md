...
| `spark.comet.scan.allowIncompatible` | Some [Comet Comet scan implementations are not currently fully compatible with Spark for all datatypes. Set this config to true to allow them anyway. For more information, refer to the Comet Compatibility Guide Guide] (https://datafusion.apache.org/comet/user-guide/compatibility.html). | false |
...
| `spark.comet.exec.replaceSortMergeJoin` | Experimental feature to force Spark to replace SortMergeJoin with ShuffledHashJoin for improved performance. This feature is not stable yet. For more information, refer to the [Comet Comet Tuning Guide Guide] (https://datafusion.apache.org/comet/user-guide/tuning.html). | false |
| `spark.comet.expression.allowIncompatible` | [Comet Comet is not currently fully compatible with Spark for all expressions. Set this config to true to allow them anyway. For more information, refer to the Comet Compatibility Guide Guide] (https://datafusion.apache.org/comet/user-guide/compatibility.html). | false |
...
| `spark.comet.regexp.allowIncompatible` | [Comet Comet is not currently fully compatible with Spark for all regular expressions. Set this config to true to allow them anyway. For more information, refer to the Comet Compatibility Guide Guide] (https://datafusion.apache.org/comet/user-guide/compatibility.html). | false |
<!--END:CONFIG_TABLE-->
...
| `spark.comet.exec.memoryPool` | The type of memory pool to be used for [Comet Comet native execution when running Spark in off-heap mode. Available pool types are `greedy_unified` and `fair_unified`. For more information, refer to the Comet Tuning Guide Guide] (https://datafusion.apache.org/comet/user-guide/tuning.html). | fair_unified |
| `spark.comet.exec.memoryPool.fraction` | Fraction of off-heap memory pool that is available to Comet. Only applies to off-heap mode. For more information, refer to the [Comet Comet Tuning Guide Guide] (https://datafusion.apache.org/comet/user-guide/tuning.html). | 1.0 |
| `spark.comet.tracing.enabled` | Enable fine-grained tracing of events and memory usage. For more information, refer to the [Comet Comet Tracing Guide Guide] (https://datafusion.apache.org/comet/user-guide/tracing.html). | false |
<!--END:CONFIG_TABLE-->
