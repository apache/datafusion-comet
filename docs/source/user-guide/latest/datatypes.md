...
hash aggregates (`min` / `max` / `sum` / `avg`), `GROUP BY`, and window functions. Remaining work is
tracked by
[#5061](https://github.com/apache/datafusion-comet/issues/5061).

| Type                    | Status | Notes                                                                                    |
| ----------------------- | ------ | ---------------------------------------------------------------------------------------- |
| `YearMonthIntervalType` | ⚠️     | Single-column sort, aggregate, `GROUP BY`, and window operators fall back.               |
| `DayTimeIntervalType`   | ⚠️     | Single-column sort, aggregate, `GROUP BY`, and window operators fall back.               |
