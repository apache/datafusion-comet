...
## EKS / IRSA: STS throttling protection (native Iceberg scan)

This is automatic; there is nothing to configure to get the protection, and it does not involve a bridge class. It applies to the **native Iceberg scan**. The raw-Parquet path is unaffected: it uses the AWS SDK default chain, which already retries and stops on a provider error rather than downgrading.
...
It stands aside whenever a higher-precedence credential source is configured -- a Comet bridge class or catalog static keys / `client.assume-role.arn`, static credentials in the environment (`AWS_ACCESS_KEY_ID` + `AWS_SECRET_ACCESS_KEY`), or a configured profile (`AWS_PROFILE`, or a shared credentials / config file such as `~/.aws/credentials` or `~/.aws/config`). These all rank ahead of web-identity in opendal's default chain, so the take-over only changes the otherwise-default behavior and never switches away from an identity -- or a profile-configured STS endpoint -- you set explicitly. On an EKS/IRSA pod none of these are normally present, so the take-over still applies there.
...
| `s3.comet.credential.webIdentity.minTtlSeconds`        | `300`   | Refresh this many seconds before expiry.                       |
| `s3.comet.credential.webIdentity.refreshJitterSeconds` | `60`    | Upper bound on the extra per-process refresh jitter.           |
...
The `s3.` prefix is required — that is how a catalog property reaches the native reader (the same route the credential-provider SPI key uses); a key without it is dropped and has no effect.
