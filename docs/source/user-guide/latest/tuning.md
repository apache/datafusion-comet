...
the TopK heap updates the predicate as its worst retained key improves. The reader can use
this predicate to skip data, and Comet also applies it to decoded batches before they reach the
heap. This residual check has its own row counters and evaluation time and can add overhead
when little data is rejected. Each native execution starts with a fresh predicate; thresholds
are never shared across tasks or sent back across Spark exchanges or JVM/Arrow boundaries.
Each partition retains
...
conversions. Attachment also skips scans with existing predicates other than direct-column
`IS NOT NULL` checks joined by `AND`, preserving their original statistics pruning. Files that
require schema adaptation beyond column remapping or missing-column literals retain their rows
for decoding, so filtering cannot hide schema-conversion errors. Incomplete Parquet column
statistics with an omitted null count are treated as unknown. The local TopK still runs normally.
