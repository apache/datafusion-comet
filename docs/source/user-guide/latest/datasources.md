...
URLs use the same shape Spark and Hadoop emit: `abfss://<container>@<account>.dfs.core.windows.net/<path>`. The account is taken from the host as written, the way Hadoop reads it from the URL authority, so `MyAcct.dfs.core.windows.net` resolves `fs.azure.account.key.MyAcct.dfs.core.windows.net` and not the lowercase spelling. The container is taken from the URL user-info as written. The `wasb`, `wasbs`, `az`, `azure`, and `adl` schemes are not supported by the native scan.
...
The Hadoop `fs.azure.*` configuration is authoritative for authentication, matching Hadoop's own ABFS driver, which reads no environment variables. When the Hadoop keys configure a complete auth mechanism (a shared account key, a SAS token, a client secret, a federated token file, or an MSI endpoint), or `fs.azure.account.oauth.provider.type` names `MsiTokenProvider`, that mechanism alone determines the identity and no `AZURE_*` environment variable is consulted: not credentials such as `AZURE_STORAGE_TOKEN`, `AZURE_STORAGE_ACCOUNT_KEY`, or `AZURE_FEDERATED_TOKEN_FILE`, and not transport settings such as `AZURE_ALLOW_HTTP`, `AZURE_PROXY_URL`, or `AZURE_STORAGE_ENDPOINT`, so the configured identity cannot be outranked, redirected, or intercepted by the environment. A partial Hadoop mechanism, such as `fs.azure.account.oauth2.token.file` or `fs.azure.account.oauth2.client.secret` without a client id and tenant, is not completed from the environment either: the scan fails with an error naming the missing keys, because Hadoop requires the client id and tenant as well and `object_store` would otherwise fall through its credential chain to the node's managed identity. A blank SAS token or credential value (`fs.azure.account.key`, `fs.azure.account.oauth2.client.id`, `fs.azure.account.oauth2.client.secret`, `fs.azure.account.oauth2.msi.tenant`, `fs.azure.account.oauth2.token.file`, and the other translated keys) is an error for the same reason, naming the exact key: `object_store` would send unsigned requests or fall through its chain. The one exception is `MsiTokenProvider`: a blank `fs.azure.account.oauth2.client.id` or `fs.azure.account.oauth2.msi.tenant` under that provider is treated as absent, because Hadoop accepts empty strings there and a system-assigned identity sets them that way, so the scan proceeds to the managed identity endpoint with no client id as Hadoop does.

When the Hadoop keys configure no auth mechanism and name no identity, the `AZURE_*` environment variables (`AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_FEDERATED_TOKEN_FILE`, `AZURE_AUTHORITY_HOST`, `AZURE_STORAGE_*`) are honored. This is what makes AKS Workload Identity work in a stock pod with no extra configuration. Transport settings from the environment apply only in this case, alongside the environment credentials. The same rule covers `AZURE_USE_AZURE_CLI`, which makes `object_store` take its token from a logged-in Azure CLI, and `AZURE_SKIP_SIGNATURE`, which makes it send unsigned requests for public containers: both are honored only when Hadoop supplies neither a mechanism nor an identity, like every other `AZURE_*` variable.

When the Hadoop keys only name the identity, via `fs.azure.account.oauth2.client.id`, `fs.azure.account.oauth2.msi.tenant`, or `fs.azure.account.oauth2.msi.authority`, or via `fs.azure.account.oauth.provider.type` ending in `WorkloadIdentityTokenProvider` together with the client id and tenant, only `AZURE_FEDERATED_TOKEN_FILE` is read from the environment, and only when `fs.azure.account.oauth2.token.file` is not set. No other `AZURE_*` variable is consulted, so an ambient account key or bearer token cannot outrank the named principal. A named principal with no token file in either place is an error naming both sources, and a borrowed token file still needs the client id and tenant from the Hadoop keys.
...
| SAS token                 | `fs.azure.sas.<container>.<account>`, `fs.azure.sas.fixed.token`                                                                                                       |

This authority extends only to the mechanisms Comet recognises. An explicit `fs.azure.account.auth.type` is Hadoop choosing a mechanism, so it is validated against the translated keys rather than ignored. It also decides which keys are read, as it does for Hadoop: only the selected mechanism's keys are translated and validated, so an OAuth client secret left in the configuration under an account-scoped `SharedKey`, or an account key under `OAuth`, is left alone, neither rejected nor passed to the store. With no auth type at all, Hadoop defaults to `SharedKey` whenever `fs.azure.account.key` (or its account-scoped form) is present, and the native scan follows suit: only the account key is read, a blank key is still an error, and OAuth or SAS keys beside it are skipped in the same way. With neither an auth type nor an account key, every mechanism's keys are read, as described above. Once `fs.azure.account.oauth.provider.type` names `ClientCredsTokenProvider`, `MsiTokenProvider` or `WorkloadIdentityTokenProvider`, only the OAuth keys that class reads in Hadoop are translated and checked, so the store sees exactly what Hadoop would read. `ClientCredsTokenProvider` reads the client id, secret and endpoint, and takes the tenant from the endpoint alone. `MsiTokenProvider` reads the MSI endpoint, tenant, client id and authority. `WorkloadIdentityTokenProvider` reads the authority, tenant, client id and token file. Settings a shared configuration keeps for another provider (a refresh token, a user name or password, an MSI tenant or endpoint, a client secret) are ignored rather than rejected or passed to the store. Hadoop ABFS mechanisms that have no native counterpart are neither translated nor allowed to fall through to the environment or to managed identity. In each of these cases no `AZURE_*` variable is consulted, and when the translated keys do not satisfy the mechanism the scan fails with an error naming the key (and, for a class name, the value) instead:

| Hadoop key                                   | Outcome in the native scan                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `fs.azure.account.auth.type` = `SharedKey`   | Accepted only with `fs.azure.account.key`                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `fs.azure.account.auth.type` = `OAuth`       | Accepted with `fs.azure.account.oauth.provider.type` ending in `MsiTokenProvider`, `WorkloadIdentityTokenProvider` (with the client id and tenant, and a token file from `fs.azure.account.oauth2.token.file` or `AZURE_FEDERATED_TOKEN_FILE`, the only environment variable read in that case), or `ClientCredsTokenProvider` (with the client secret), or with no provider class and a client secret or token file; any other provider class is rejected, naming the class |
| `fs.azure.account.auth.type` = `Custom`      | Loads a custom token provider class, which the native scan cannot run                                                                                                                                                                                                                                                                                                                                                                                                        |
| `fs.azure.account.auth.type` = `SAS`         | Rejected unless `fs.azure.sas.fixed.token` (or a container-scoped SAS token) is also set                                                                                                                                                                                                                                                                                                                                                                                     |
| `fs.azure.account.auth.type` = anything else | Rejected, naming the value; the native scan supports `SharedKey`, `OAuth` and `SAS`                                                                                                                                                                                                                                                                                                                                                                                          |
| `fs.azure.sas.token.provider.type`           | Loads a SAS token provider class                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `fs.azure.account.keyprovider`               | Loads an account key provider class. `org.apache.hadoop.fs.azurebfs.services.SimpleKeyProvider`, the class Hadoop uses when this key is unset, is accepted with `fs.azure.account.key` when spelled exactly, with no surrounding whitespace                                                                                                                                                                                                                                  |
| `fs.azure.account.oauth2.refresh.token`      | Refresh-token OAuth flow. Ignored when `fs.azure.account.oauth.provider.type` names a supported provider class                                                                                                                                                                                                                                                                                                                                                               |
| `fs.azure.account.oauth2.user.name`          | User name and password OAuth flow. Ignored when `fs.azure.account.oauth.provider.type` names a supported provider class                                                                                                                                                                                                                                                                                                                                                      |
| `fs.azure.account.oauth2.user.password`      | User name and password OAuth flow. Ignored when `fs.azure.account.oauth.provider.type` names a supported provider class                                                                                                                                                                                                                                                                                                                                                      |

The account-scoped forms of these keys (`<key>.<account>.dfs.core.windows.net` and so on) are rejected the same way.

### Hadoop-to-object_store key mapping

Internally, each Hadoop key is translated into a specific `AzureConfigKey` on the underlying `object_store` `MicrosoftAzureBuilder`. Only keys the selected mechanism reads are translated, and under a supported `fs.azure.account.oauth.provider.type` only the OAuth keys that provider class reads:
...
Hadoop keys beyond these are not translated. `fs.azure.account.oauth.provider.type` is validated whenever it is set and OAuth is the mechanism read, that is with `fs.azure.account.auth.type=OAuth`, or with no auth type and no `fs.azure.account.key` (a key with no auth type selects `SharedKey`, Hadoop's default, and the class is then ignored): a class ending in `MsiTokenProvider` stands alone, `WorkloadIdentityTokenProvider` needs the client id and tenant, `ClientCredsTokenProvider` needs the client secret principal, any other class is an error naming the key and the class, and a blank value is an error naming the key. A class matches only by its exact simple name, bare or as the last segment of a qualified name. It, `fs.azure.account.auth.type`, and the unsupported mechanism keys listed under [Supported Authentication](#supported-authentication) are read only to decide whether Hadoop configures an auth mechanism, and whether it is one the native scan can build. The storage account and container always come from the URL.
...
The SAS namespace follows the same shape with the container inlined into the key name: `fs.azure.sas.<container>.<account>.dfs.core.windows.net`, then `fs.azure.sas.<container>.<account>.blob.core.windows.net`, then `fs.azure.sas.<container>.<account>`. When none of those is set, the account-level `fs.azure.sas.fixed.token` is probed in the account-scoped order above, so a container-scoped token always wins over a fixed token. A SAS token that is empty or only whitespace is rejected rather than treated as unset, and a blank container-scoped key is rejected even when a valid `fs.azure.sas.fixed.token` is set: the fixed token is never used as a fallback for it.
