...
URLs use the same shape Spark and Hadoop emit: `abfss://<container>@<account>.dfs.core.windows.net/<path>`. The account is taken from the host and lowercased before any key lookup, so `MyAcct.dfs.core.windows.net` resolves `fs.azure.account.key.myacct.dfs.core.windows.net`; the container is taken from the URL user-info as written, which matches Hadoop, and Azure requires container names to be lowercase anyway. The `wasb`, `wasbs`, `az`, `azure`, and `adl` schemes are not supported by the native scan.
