...
| `pmod` | ✅ |  |
| `positive` | ✅ |  |
| `pow` | ✅ | Runs through the JVM codegen dispatcher by default; the native path (correctness issues) is opt-in via allowIncompatible |
| `power` | ✅ | Runs through the JVM codegen dispatcher by default; the native path (correctness issues) is opt-in via allowIncompatible |
| `radians` | ✅ |  |
| `rand` | ✅ |  |
| `randn` | ✅ |  |
