...
By default, Comet runs a Spark-compatible implementation of `MapFromEntries`. Set `spark.comet.expression.MapFromEntries.allowIncompatible=true` to use Comet's faster native implementation instead, which has the following differences from Spark:
...
The following incompatibilities cause `StringToMap` to fall back to Spark by default. Set `spark.comet.expression.StringToMap.allowIncompatible=true` to enable Comet acceleration despite these differences.
