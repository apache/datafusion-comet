 ...
 *     a `CometDeltaScanMarker` (carrying a `DeltaScanMetadata` field). Once the serde unit lands,
 *     the marker mixes in core's `CometContribScanMarker` and `CometExecRule` routes it through
 *     [[CometDeltaNativeScan]] by a plain type test; until then the marker is left in the plan
 *     and executes as a vanilla Delta read.
 *
 * SPI surfaces used:
 ...
 */
/**
 * Static entry points for Delta scan detection / transformation. Reached from core only through
 * [[DeltaScanRuleContrib]], the `CometScanContrib` service provider, which core discovers via the
 * JDK `ServiceLoader` -- and only when the contrib's classes and its
 * `META-INF/services/org.apache.comet.rules.CometScanContrib` file are bundled into
 * `comet-spark.jar` (i.e. when Maven was invoked with `-Pcontrib-delta`).
 */
