 ...
 * is any *extension/discovery* SPI -- core's `CometExecRule` reaches this object through
 * `CometDeltaScanMarker.scanHandler` (the marker mixes in core's `CometContribScanMarker` and
 * carries its own handler, so core needs no lookup at all). The wire format is core's generic
 * `OpStruct::ContribScan` envelope carrying a `DeltaScan` -- see [[DeltaContribScan]].
 */
object CometDeltaNativeScan extends CometOperatorSerde[CometDeltaScanMarker] with Logging {
  ...
  override def convert(
    ...
    // Pack the DeltaScan into core's generic `contrib_scan` envelope. Core's planner dispatches
    // the single `OpStruct::ContribScan` arm on `type_url` to the `contrib-delta`-gated handler,
    // so core names no Delta type. See `DeltaContribScan` for the type URL contract.
    builder.clearChildren()
    Some(DeltaContribScan.set(builder, deltaScanBuilder.build()).build())
  }
