...
case class CometDeltaNativeScanExec(
  ...
  @transient private lazy val commonBytes: Array[Byte] = {
    // The DeltaScan inside OpStruct's generic `contrib_scan` envelope carries the common block.
    DeltaContribScan.get(nativeOp).getCommon.toByteArray
  }
...
object CometDeltaNativeScanExec {
  ...
  def computeSourceKey(nativeOp: Operator): String = {
    val common = DeltaContribScan.get(nativeOp).getCommon
    val components = Seq(
      common.getTableRoot,
