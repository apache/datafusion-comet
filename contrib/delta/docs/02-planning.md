...
The Delta contrib plugs into that rule through core's format-agnostic
`CometScanContrib` SPI. From `spark/.../CometScanRule.scala`, inside the
per-`FileSourceScanExec` match on `HadoopFsRelation`:
...
`CometScanContrib` (`spark/.../rules/CometScanContrib.scala`) is a trait plus a
registry object in core's `org.apache.comet.rules` package. It names no format:

1. Implementations are discovered once per JVM with the JDK `ServiceLoader`,
   from `META-INF/services/org.apache.comet.rules.CometScanContrib` resources
   on the classpath — the same mechanism core uses for `PlanDataInjector`.
2. The trait has three hooks, each defaulting to `None`: `tryTransformV1`
   (`FileSourceScanExec`), `tryTransformV2` (`BatchScanExec`), and
   `tryTransformRowScan` (`RowDataSourceScanExec`, used for Change Data Feed —
   see [03-native-execution.md](03-native-execution.md)).
3. The registry returns the first contrib to claim the scan: `Some(handled)` if
   a contrib either claimed it or declined it via its own `withFallbackReason`
   fallback marker; `None` if no contrib recognises the relation.

On a default build no service file is packaged, so the registry is empty, every
hook returns `None`, and core carries zero Delta surface. The contrib's provider
is `DeltaScanRuleContrib`, a thin adapter that delegates to `DeltaScanRule`
(a `class`, not an `object`, because `ServiceLoader` needs a public no-arg
constructor).
...
Delta-specific planning info in a `DeltaScanMetadata` field. The marker mixes in
core's `CometContribScanMarker` and carries its own `scanHandler`, so
`CometExecRule` matches it with a plain type test and routes it through
`convertToComet` with the `CometDeltaNativeScan` serde — no class-name match and
no reflective handler lookup in core. If conversion declines, the marker falls
back to executing the wrapped vanilla Spark scan, so leaving it in the plan is
always safe.
...
is written on the JVM side; `DELTA_SCAN_TYPE_NAME` in
`native/core/src/execution/planner/delta_scan.rs` is its native counterpart.

The per-partition task arrays ride alongside via
