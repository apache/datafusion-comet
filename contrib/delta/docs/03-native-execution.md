...
`DeltaCDFRelation`. `CometExecRule` hands every `RowDataSourceScanExec` to core's
format-agnostic `CometScanContrib.tryTransformRowScan` hook; the contrib's
`DeltaScanRuleContrib` is the one that recognises the relation by class name and
routes it through `CometDeltaNativeScan.convertCdf` →
`CometDeltaCdfScanExec`, which drives `TableChanges` natively. Core inspects
neither the relation nor the contrib. The inclusive
...
`transformV1IfDelta` catches it, logs a warning and declines via
