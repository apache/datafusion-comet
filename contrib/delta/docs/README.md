...
Comet's existing plan-rewrite rule (`CometScanRule`) through core's
format-agnostic `CometScanContrib` SPI (discovered via the JDK `ServiceLoader`),
recognises Delta `LogicalRelation`s, and substitutes them with a native scan
node. Driver-side, `delta-kernel-rs`
...
logical schemas, encoded into a `DeltaScan` message inside core's generic
`ContribScan` proto envelope and shipped to executors.
...
```
spark/src/main/scala/org/apache/comet/rules/CometScanContrib.scala   # generic contrib scan SPI + marker trait (core)
spark/src/main/scala/org/apache/comet/rules/CometScanRule.scala      # calls CometScanContrib.tryTransformV1/V2
spark/src/main/scala/org/apache/comet/rules/CometExecRule.scala      # marker→native + tryTransformRowScan (CDF)
native/proto/src/proto/operator.proto                                # generic ContribScan envelope + DeltaScan messages
native/core/src/execution/planner/delta_scan.rs                      # Delta handler for the ContribScan arm (contrib-delta-gated)
