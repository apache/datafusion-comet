...
│       │                                                              │
│       │   CometScanContrib SPI -> DeltaScanRuleContrib               │
│       ▼                                                              │
...
- No `META-INF/services/org.apache.comet.rules.CometScanContrib` resource is
  packaged, so core's `ServiceLoader` registry is empty and every
  `CometScanContrib` hook returns `None` for the JVM lifetime
- The Delta handler in `native/core/src/execution/planner/delta_scan.rs` is
  `#[cfg(feature = "contrib-delta")]`-gated and compiles out of the dylib; the
  generic `OpStruct::ContribScan` arm remains but can only report that no
  compiled-in contrib handles the incoming `type_url`
- The `ContribScan` envelope is present in the schema but never emitted
