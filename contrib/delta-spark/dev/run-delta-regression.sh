...
# Canonicalize each entry to an absolute path before the cd below, for the same
# reason as the WORKDIR normalization above: the injected sbt `file(p)` resolves
# a relative COMET_EXTRA_JARS entry beneath $DELTA_DIR, not the caller's directory,
# once we've already changed into the Delta checkout.
IFS=',' read -ra _jars <<< "$COMET_JARS"
_jars_abs=()
for j in "${_jars[@]}"; do
  [ -f "$j" ] || { echo "COMET_JARS entry not found: $j" >&2; exit 2; }
  _jars_abs+=("$(cd "$(dirname "$j")" && pwd)/$(basename "$j")")
done
COMET_JARS=$(IFS=','; echo "${_jars_abs[*]}")
