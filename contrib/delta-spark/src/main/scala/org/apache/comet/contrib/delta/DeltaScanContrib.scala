...
class DeltaScanContrib extends CometScanContrib with Logging {
  ...
  override def tryTransformV1(
    ...
    // Populated by declineReason on the claimable path only, and reused by convert below so a
    // claimed scan does not recompute the Hadoop conf or the DV descriptors a second time.
    val claimMemo = new DeltaScanSupport.DeltaClaimMemo
    DeltaScanSupport.declineReason(plan, scanExec, scanHelper, claimMemo) match {
      case Some(reason) =>
        Some(withFallbackReason(scanExec, reason))
      case None =>
        CometDeltaNativeScan.convert(scanExec, scanHelper, claimMemo) match {
          case Some(nativeOp) =>
            logDebug(
