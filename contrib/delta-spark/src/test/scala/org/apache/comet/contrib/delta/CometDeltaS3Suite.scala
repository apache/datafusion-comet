...
/**
 * MinIO-backed integration coverage for multi-bucket Delta shapes: a real two-bucket shallow
 * clone, which a single-bucket `withTempPath` table can never produce, because it is
 * `DeltaTable`'s CLONE machinery -- not test fixturing -- that leaves some `AddFile` entries
 * pointing at the source table's absolute location while new files land under the clone's own
 * root.
 *
...
class CometDeltaS3Suite extends CometDeltaTestBase with CometS3TestBase with Logging {
  ...
  /**
   * A bucket touched by no other test in this suite: the native S3 object-store cache
   * (`object_store_cache` in parquet_support.rs) is process-wide and keyed per bucket, so reusing
   * [[testBucketName]] for the `${...}` forwarding test below risks silently passing against a
   * store handle another test already warmed with plain credentials, rather than actually forcing
   * a fresh credential derivation through the substituted `${...}` value.
   */
  private val reviewRefBucketName = "comet-delta-review-ref"
  ...
  override def beforeAll(): Unit = {
    ...
    if (dockerAvailable) {
      ...
      Try {
        ...
        createBucketIfNotExists(reviewRefBucketName)
      } match {
        case Success(_) =>
  ...
  test("shallow clone across buckets + append declines with the multi-store reason") {
    ...
    // authorities -- exactly the shape the multi-store decline gate exists for, since the shared
  ...
  test(
    "clone across buckets + DELETE on the clone reads correct rows natively " +
      "(cold cross-bucket deletion-vector store)") {
    assume(dockerAvailable, "Docker is not available; skipping MinIO-backed Delta test")
    ...
    // relocates data it did not modify. That is the cold cross-bucket deletion-vector-store bug
    // shape: attaching the access plan nested a `Handle::block_on` call that built the
    // (previously untouched, so cold) bucket-B object store for the sidecar from inside an
    // already-running Tokio runtime, which panics.
    //
    // It is also, deliberately, NOT the shape the multi-store decline gate catches: that gate
    ...
    // own authority is bucket B. That distinction is worth stating here because it is
    // the one property that makes this test exercise the cold cross-bucket deletion-vector-store
    // path instead of re-proving the multi-store decline gate.
    ...
    // the cold cross-bucket deletion-vector-store code path at all, while
    // `checkDeltaNativeScanAnswer` below would still pass.
    ...
    // or canonically-empty descriptors are excluded first (`cardinality == 0` is the
    // EMPTY-descriptor characterization: no rows deleted, so no on-disk sidecar exists):
    // DeletionVectorDescriptor#absolutePath's isOnDisk precondition
  ...
  test(
    ...
    // Mutate the session's shared hadoopConfiguration directly (mirroring the set-then-restore
    // shape CometDeltaNativeScanSuite's viewfs gate test uses for the same reason: these are
    // plain Hadoop Configuration entries, not SQLConf, so withSQLConf cannot round-trip them).
    // Aliasing the real MinIO credentials behind review.access/review.secret and pointing
    // fs.s3a.access.key/fs.s3a.secret.key at them via ${...} reproduces exactly the shape
    // PART 1 fixed: Configuration#get expands the reference to the real credential, and
    // NativeConfig.extractObjectStoreOptions must forward that EXPANDED value, not the literal
    // "${review.access}" string, or the native S3 client would authenticate with garbage.
    val hadoopConf = spark.sparkContext.hadoopConfiguration
    val priorAccessKey = Option(hadoopConf.get("fs.s3a.access.key"))
    ...
    try {
      ...
      hadoopConf.unset("review.access")
      hadoopConf.unset("review.secret")
      priorAccessKey match {
        case Some(v) => hadoopConf.set("fs.s3a.access.key", v)
        case None => hadoopConf.unset("fs.s3a.access.key")
      }
      priorSecretKey match {
        case Some(v) => hadoopConf.set("fs.s3a.secret.key", v)
        case None => hadoopConf.unset("fs.s3a.secret.key")
      }
    }
  }
}
