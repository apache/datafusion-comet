...
class DeltaScanContribSuite extends CometDeltaTestBase {
  ...
  test(
    "parentDirectories maps selected files to their distinct directories in first-seen order") {
    val dirs = DeltaScanSupport.parentDirectories(
      Seq(
        new URI("file:///tmp/dir%0A/data/part-0.parquet"),
        new URI("file:///tmp/dir%0A/data/part-1.parquet"),
        new URI("file:///tmp/other/deletion_vector_a.bin")))
    assert(
      dirs == Seq(new URI("file:///tmp/dir%0A/data"), new URI("file:///tmp/other")),
      s"unexpected directories: $dirs")
    // The rejected directory is what the probe sees, not the file name. Hadoop's Path renders
    // an authority-less file URI as `file:/...`, so match its own rendering.
    val reason = DeltaScanSupport.objectStoreRejectedPathReason(dirs, Set("hdfs"))
    assert(reason.exists(_.contains(s"cannot open path '${dirs.head}'")), s"reason: $reason")
  }
