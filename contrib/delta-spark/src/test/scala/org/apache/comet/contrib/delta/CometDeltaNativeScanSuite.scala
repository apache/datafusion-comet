...
class CometDeltaNativeScanSuite extends CometDeltaTestBase {
  ...
  test("table root under a directory whose name contains a newline falls back to Spark") {
    // object_store recognizes the `file` scheme but rejects the control character in the
    // directory name (`%0A` in the URI), so native execution could not open the table where
    // Spark's Hadoop-backed reader can. The claim gate must decline before native planning.
    withTempPath { dir =>
  ...
  test("shallow clone whose source data files sit under a newline directory falls back") {
    // The clone's own root is an ordinary path, so only the selected data files (resolved to
    // the source table's directory) carry the rejected segment: this exercises the
    // selected-paths probe, not the root gate. The reason names the first such complete path,
  ...
  test("converted Parquet table with a newline in a data file basename falls back to Spark") {
    // CONVERT TO DELTA keeps the existing Parquet file names, so the rejected character sits in
    // the basename rather than a directory segment: the table root and every parent directory
    // pass the path probe, and only a check of the complete selected path can decline.
    withTempPath { dir =>
      val path = new File(dir, "data").getAbsolutePath
      spark.range(0, 100).repartition(2).write.parquet(path)
      val original = new File(path).listFiles().filter(_.getName.endsWith(".parquet")).head
      ...
      checkSparkAnswerAndFallbackReason(
        df,
        s"Native Delta scan cannot open path 'file:$path/part-00000%0A.snappy.parquet': " +
