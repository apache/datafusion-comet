...
use datafusion_comet_shuffle::{
    ...
    PartitionOffsets, RoundRobinStrategy, ShuffleBlockWriter, ShuffleCodecContext,
    ShufflePartitioner, ShufflePartitionerMetrics, ShuffleWriterExec,
};
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    //
    // The repartitioner is built in the setup closure, not the timed one: `try_new` sizes the
    // row scratch from the strategy, so timing it would charge `hash_all_columns` ~64 KB of
    // per-iteration allocation that `whole_batch` does not pay and bias the comparison.
    for (label, strategy) in &round_robin_strategies {
        group.bench_function(
...
fn ipc_context_reuse_benchmark(c: &mut Criterion) {
    ...
    for rows in [128, 8192] {
        for (name, batch) in [
            ...
            ("nested", nested_schema_batch(rows, 4, 6)),
        ] {
            for codec in [
                CompressionCodec::None,
                CompressionCodec::Lz4Frame,
