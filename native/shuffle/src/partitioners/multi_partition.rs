...
mod tests {
    ...
    async fn check_spill_write_error_releases_buffered_memory(consume_before_failure: bool) {
        ...
        repartitioner
            .buffer_partitioned_batch_may_spill(batch.slice(0, 2), Some(&[0, 1]), &[0, 1, 2])
            .await
            .unwrap();
        ...
        repartitioner
            .buffer_partitioned_batch_may_spill(batch.slice(2, 2), Some(&[0, 1]), &[0, 1, 2])
            .await
            .unwrap();
    ...
    async fn rejected_growth_counts_existing_reservation_and_unreserved_input() {
        ...
        for share_buffer in [false, true] {
            for (fail, consume_before_failure) in [(false, false), (true, false), (true, true)] {
                ...
                repartitioner
                    .buffer_partitioned_batch_may_spill(batch, Some(&row_indices), &[0, 32, 64])
                    .await
                    .unwrap();
