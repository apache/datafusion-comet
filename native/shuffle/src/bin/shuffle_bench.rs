    ...
    rt.block_on(async {
        ...
        for task_id in 0..args.concurrent_tasks {
            ...
            let handle = tokio::spawn(async move {
                ...
                let shuffle_mode = match shuffle_mode_str.as_str() {
                    "buffered" => ShuffleMode::Buffered,
                    _ => ShuffleMode::Immediate,
                };
        ...
        // Clean up task output directories
        for task_id in 0..args.concurrent_tasks {
            let task_dir = args.output_dir.join(format!("task_{task_id}"));
