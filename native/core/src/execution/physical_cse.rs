...
//!
//! Identifies repeated subexpressions within a `ProjectionExec`'s expression
//! list and rewrites the plan to compute them once via an intermediate
//! projection.
...
/// Identify sub-expressions that appear 2+ times across the projection's
...
fn find_common_subexprs(
    ...
    for proj_expr in exprs {
        collect_subexprs(&proj_expr.expr, &mut counts);
    }
