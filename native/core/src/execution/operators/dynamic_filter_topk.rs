...
impl DynamicFilterTopKExec {
    ...
    fn build_runtime_sort(&self) -> Result<RuntimeDynamicFilterTopK> {
        ...
        // The reader reports its own savings. Count residual filtering after decoding,
        // before DataFusion applies its own TopK predicate and updates the heap.
        let input = Arc::new(DynamicFilterExec::new(
            input,
            Arc::clone(&predicate),
            self.metrics.clone(),
            "dynamic_filter_topk",
