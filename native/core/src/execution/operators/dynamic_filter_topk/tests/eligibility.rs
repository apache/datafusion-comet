...
/// File statistics can prune before the per-file schema adapter runs, even when
/// all statistics are unknown and DataFusion enables early stopping from them.
#[tokio::test]
    ...
    config.file_groups = config
        ...
        .map(|group| {
            group
                ...
                .iter()
                .cloned()
                .map(|file| file.with_statistics(Arc::new(Statistics::new_unknown(&scan.schema()))))
                .collect::<Vec<_>>()
                .into()
        })
        .collect();
    let scan = Arc::new(scan.as_ref().clone().with_data_source(Arc::new(config)));
    let plan = Arc::new(wrapper(&sort(scan, 1, SortOptions::default()), &session));
    let batches = collect(Arc::clone(&plan) as _, session.task_ctx())
        .await
        .unwrap();
    assert_eq!(keys(&batches), vec![Some(0)]);
    assert_eq!(
