...
/// into `from_ffi_and_data_type` itself. After this function materializes the batch, `ScanExec`
/// passes every column through `import_column`, which validates and decodes string data before
/// native execution can read it. `BroadcastInputExec` validates strings before cache copying and
/// uses the same decoder for ordinary replay. Any future consumer must preserve that validation.
