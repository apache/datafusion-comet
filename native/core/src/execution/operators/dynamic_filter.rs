...
//! Apply a live runtime predicate to decoded batches and report its owner's metrics.
...
/// A task-local consumer of a join or TopK runtime filter.
#[derive(Debug)]
...
impl ExecutionPlan for DynamicFilterExec {
    ...
    fn reset_state(self: Arc<Self>) -> Result<Arc<dyn ExecutionPlan>> {
        // Never retain a previous execution's domain or threshold. A reset consumer
        // safely bypasses filtering; its owning join or TopK constructs a fresh,
        // connected producer and consumer for each ordinary Spark task attempt.
    ...
    fn execute(
        ...
        let [key] = children.as_slice() else {
            return internal_err!("CometDynamicFilterExec requires one key column");
        };
        let Some(key) = key.downcast_ref::<Column>() else {
            return internal_err!("CometDynamicFilterExec requires a direct key column");
        };
        ...
        // Share only these dedicated metrics with the owning join or TopK.
        // Do not add input/output counts or baseline elapsed time to its metrics.
        ...
        let stream = input.map(move |batch| {
            ...
            match predicate.evaluate(&key_batch)? {
                // The producer has not published a useful predicate yet.
                ...
                ColumnarValue::Array(mask) => {
                    ...
                    let filtered = if mask.len() == input_rows && mask.true_count() == input_rows {
                        // Improving TopK thresholds often keep every row in a batch.
                        // Preserve the original payload arrays in that common case.
                        batch
                    } else {
                        filter_record_batch(&batch, mask)?
                    };
                    evaluated.add(input_rows);
                    pruned.add(input_rows - filtered.num_rows());
                    Ok(filtered)
                }
                _ => internal_err!("Dynamic filter must evaluate to a Boolean"),
            }
        });
