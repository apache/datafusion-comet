...
/// CometFilterExec evaluates a boolean predicate against all input batches to determine which rows to
...
impl CometFilterExec {
    /// Create a CometFilterExec on an input
    pub fn try_new(
        predicate: Arc<dyn PhysicalExpr>,
        input: Arc<dyn ExecutionPlan>,
    ) -> Result<Self> {
    ...
    /// Calculates `Statistics` for `CometFilterExec`, by applying selectivity (either default, or estimated) to input statistics.
    ...
    fn compute_properties(
        ...
        eq_properties = eq_properties.add_constants(Self::extend_constants(input, predicate));
    ...
    fn fmt_as(&self, t: DisplayFormatType, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match t {
            DisplayFormatType::Default | DisplayFormatType::Verbose => {
                write!(f, "CometFilterExec: {}", self.predicate)
            }
        }
    }
}
...
impl ExecutionPlan for CometFilterExec {
    ...
    fn with_new_children(
        ...
        CometFilterExec::try_new(Arc::clone(&self.predicate), children.swap_remove(0))
            .and_then(|e| {
    ...
    fn execute(
        ...
        trace!("Start CometFilterExec::execute for partition {} of context session_id {} and task_id {:?}", partition, context.session_id(), context.task_id());
...
/// The CometFilterExec streams wraps the input iterator and applies the predicate expression to
