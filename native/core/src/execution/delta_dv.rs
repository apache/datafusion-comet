...
pub fn build_access_plan(
    ...
    for (idx, &num_rows) in row_group_row_counts.iter().enumerate() {
        ...
        if num_rows < 0 {
            return Err(GeneralError(format!(
                "Parquet footer reports a negative row count ({num_rows}) for row group {idx}"
            )));
        }
...
mod tests {
    ...
    fn resolve_store(runtime_env: &Arc<RuntimeEnv>, url: &str) -> (Arc<dyn ObjectStore>, Path) {
        ...
        let (store_url, path, _) = prepare_object_store_with_configs(
    ...
    #[test]
    fn access_plan_rejects_negative_row_count_reported_by_a_corrupt_footer() {
        // A corrupt footer can report a negative row count for a row group. Round-trip through
        // the real parquet-crate RowGroupMetaData builder (`into_builder`, reusing a real row
        // group's own column metadata rather than a bare negative literal) to prove the guard
        // fires on the exact shape a corrupt footer would produce, not just an arbitrary i64.
        let tmp = tempfile::tempdir().unwrap();
        let path = tmp.path().join("f.parquet");
        write_parquet(&path, 10);
        let metadata = read_metadata_with_page_index(&path);
        let corrupted = metadata
            .row_group(0)
            .clone()
            .into_builder()
            .set_num_rows(-5)
        ...
        assert!(
            msg.contains("row group 0"),
            "expected the row group index: {msg}"
        );
    }
    ...
    /// Replays the page-index-pruning path that drives peak memory the highest: clones
    ...
    /// `num_rows` is chosen just above a power of two (at test scale, `1,048,577` rows) so the
    /// intersection's `next_power_of_two` capacity jump is real and visible, not accidentally
    /// exact.
    ///
    ...
    // -----------------------------------------------------------------------------------------
    // Malformed-input hardening matrix: every way a deletion-vector blob can be corrupted
    // (truncation, CRC, magic, length lies, cardinality lies, and general bit-flip fuzzing) must
    // yield a clean `Err`, NEVER a panic and never a silently wrong answer.
    // -----------------------------------------------------------------------------------------

    /// Runs `f` under `catch_unwind`, failing the test with `context` if it panics. Every
    /// malformed-input case below routes through this so a panic surfaces as an attributable test
    /// failure instead of aborting the whole test binary silently at whichever case triggered it.
    fn assert_no_panic<T>(context: &str, f: impl FnOnce() -> T + std::panic::UnwindSafe) -> T {
        match std::panic::catch_unwind(f) {
            Ok(result) => result,
            Err(_) => panic!("panicked while decoding malformed input: {context}"),
        }
    }

    /// A valid on-disk-framed blob (`[i32 BE size][data][i32 BE crc]`) plus its unframed `data`
    /// payload (the portable-format `[i32 LE magic][RoaringTreemap bytes]`, the same bytes an
    /// inline DV descriptor would carry directly), shared by every malformed-input case below so
    /// each corruption starts from one known-good baseline.
    fn valid_dv_fixture() -> (Vec<u8>, Vec<u8>) {
        let deleted: RoaringTreemap = [1u64, 5, 6, 7, 1000, (3u64 << 32) + 42]
            .into_iter()
            .collect();
    ...
    /// be rejected cleanly by `unframe_dv_blob`, never panic -- covers every truncation point in
    /// one deterministic sweep rather than a few hand-picked lengths.
    #[test]
    fn unframe_rejects_every_truncation_length() {
        let (blob, data) = valid_dv_fixture();
        let expected_size = data.len();
        for len in 0..blob.len() {
            let truncated = &blob[..len];
            let result = assert_no_panic(&format!("on-disk blob truncated to {len} bytes"), || {
                unframe_dv_blob(truncated, expected_size)
            });
            assert!(
                result.is_err(),
                "truncating the on-disk blob to {len}/{} bytes should be rejected",
                blob.len()
            );
        }
    }

    /// (1, inline-DV path) `attach_access_plan` feeds an inline descriptor's `inline_data`
    /// straight to `deserialize_dv_bitmap`, skipping `unframe_dv_blob` entirely -- it carries no
    /// `[size][data][crc]` framing, just `[i32 LE magic]...`. Every truncation length of that
    /// unframed payload must also be handled cleanly: either a clean `Err`, or -- if a truncated
    /// prefix happens to still parse -- a well-formed treemap that `build_access_plan` can
    /// consume without panicking. Never a panic in either step.
    #[test]
    fn deserialize_rejects_every_truncation_length_of_inline_payload() {
        let (_blob, data) = valid_dv_fixture();
        for len in 0..data.len() {
            let truncated = &data[..len];
            let context = format!("inline payload truncated to {len} bytes");
            let result = assert_no_panic(&context, || deserialize_dv_bitmap(truncated));
            if let Ok(treemap) = result {
                let max_row = treemap
                    .max()
    ...
    /// (2) Flipping each byte of the CRC field individually must be rejected as a checksum
    /// mismatch. XORing with `0xFF` guarantees the flipped byte differs from its original value
    /// at that position, so every flip actually corrupts the checksum -- it can never coincide
    /// with the real value by construction.
    #[test]
    fn unframe_rejects_every_crc_byte_flip() {
        let (blob, data) = valid_dv_fixture();
        let expected_size = data.len();
        let crc_start = blob.len() - 4;
        for i in crc_start..blob.len() {
            let mut corrupted = blob.clone();
            corrupted[i] ^= 0xFF;
            let context = format!("CRC byte {i} flipped");
            let result = assert_no_panic(&context, || unframe_dv_blob(&corrupted, expected_size));
            let err = result.unwrap_err();
            assert!(
                format!("{err}").contains("checksum"),
                "{context} should be reported as a checksum mismatch: {err}"
            );
        }
    }

    /// (3) A magic number that matches neither known format must be rejected by name -- checked
    /// against both obviously-wrong values and the bitwise complement of each real magic (which,
    /// by construction, can never accidentally equal either real magic).
    #[test]
    fn deserialize_rejects_corrupted_magic() {
        let (_blob, data) = valid_dv_fixture();
        let payload = &data[4..]; // magic-stripped body, reused under every corrupted magic
        for bad_magic in [0i32, 1, -1, i32::MAX, !PORTABLE_MAGIC, !NATIVE_MAGIC] {
            assert_ne!(bad_magic, PORTABLE_MAGIC);
            assert_ne!(bad_magic, NATIVE_MAGIC);
            let mut corrupted = bad_magic.to_le_bytes().to_vec();
            corrupted.extend_from_slice(payload);
            let context = format!("magic corrupted to {bad_magic}");
            let result = assert_no_panic(&context, || deserialize_dv_bitmap(&corrupted));
            let err = result.unwrap_err();
            assert!(
    ...
    /// (4a) A declared size larger than the buffer actually holds must be rejected as truncated
    /// -- not read out of bounds, not panic -- even when the descriptor's `expected_size` agrees
    /// with the (lied-about) declared size, so it is the truncation check, not the size-mismatch
    /// check, that has to catch it.
    #[test]
    fn unframe_rejects_size_field_larger_than_buffer() {
        let (_blob, data) = valid_dv_fixture();
        let lie = data.len() + 1_000_000; // declares far more data than the buffer holds
        let mut lied_blob = (lie as i32).to_be_bytes().to_vec();
        lied_blob.extend_from_slice(&data);
        lied_blob.extend((crc32fast::hash(&data) as i32).to_be_bytes());
        let result = assert_no_panic("size field lies larger than the buffer", || {
            unframe_dv_blob(&lied_blob, lie)
        });
        let err = result.unwrap_err();
        assert!(
    ...
    /// (4b) A declared size smaller than the data actually written shifts which bytes get hashed
    /// as the CRC input, so it must surface as a checksum mismatch -- never a panic, never a
    /// successful decode of a differently-sliced payload.
    #[test]
    fn unframe_rejects_size_field_smaller_than_actual_data() {
        let (_blob, data) = valid_dv_fixture();
        let lie = data.len() - 4; // declares less data than was actually written
        let mut lied_blob = (lie as i32).to_be_bytes().to_vec();
        lied_blob.extend_from_slice(&data);
        lied_blob.extend((crc32fast::hash(&data) as i32).to_be_bytes());
        let result = assert_no_panic("size field lies smaller than actual data", || {
            unframe_dv_blob(&lied_blob, lie)
        });
        let err = result.unwrap_err();
        assert!(
    ...
    /// (5) `validate_cardinality` must reject absurd cardinality claims in BOTH directions -- far
    /// too high (a stale descriptor claiming millions of deletions for a handful of actual bits)
    /// and far too low (zero or negative expected against many actual bits) -- and must never
    /// panic, including when a negative `expected` (an `i64`) is cast to the `u64` comparison
    /// `deleted.len()` uses.
    #[test]
    fn validate_cardinality_rejects_absurd_mismatches_in_both_directions() {
        let deleted: RoaringTreemap = (0u64..1000).collect(); // 1000 actual deletions

        for (context, expected) in [
            ("claimed far too high", i64::MAX),
            ("claimed far too low (zero)", 0i64),
            ("claimed negative", -1i64),
        ] {
            let result = assert_no_panic(context, || {
                validate_cardinality("f.parquet", expected, &deleted)
            });
            let err = result.unwrap_err();
            assert!(
        ...
        // Reverse imbalance: an empty bitmap against a huge claimed cardinality.
        let empty = RoaringTreemap::new();
        let result = assert_no_panic("empty bitmap vs huge claimed cardinality", || {
            validate_cardinality("f.parquet", 1_000_000_000, &empty)
        });
        let err = result.unwrap_err();
        assert!(
            format!("{err}").contains("cardinality"),
    ...
    /// (6) Single-bit-flip fuzz sweep over one valid on-disk-framed blob: for every bit position,
    /// flip it and run the FULL decode pipeline (`unframe_dv_blob` then `deserialize_dv_bitmap`).
    /// Every outcome must be either a clean `Err` or a successfully-decoded, well-formed treemap
    /// that `build_access_plan` can consume without panicking -- NEVER a panic in either step.
    /// Bounded to one pass over one blob's bits, so runtime stays well under 5s.
    #[test]
    fn single_bit_flip_sweep_never_panics() {
        let (blob, data) = valid_dv_fixture();
        let expected_size = data.len();
        let mut checked = 0usize;

        for byte_idx in 0..blob.len() {
            for bit in 0u8..8 {
                let mut corrupted = blob.clone();
                corrupted[byte_idx] ^= 1 << bit;
                let context = format!("byte {byte_idx} bit {bit} flipped");
                checked += 1;

                let unframed = assert_no_panic(&context, || {
                    unframe_dv_blob(&corrupted, expected_size).map(|d| d.to_vec())
                });
                let Ok(unframed_data) = unframed else {
                    continue;
                };

                let decoded = assert_no_panic(&context, || deserialize_dv_bitmap(&unframed_data));
                if let Ok(treemap) = decoded {
                    // A "VALID selection": consuming the decoded treemap downstream must not
                    // panic either, whatever its contents happen to be. `checked_add` avoids an
                    // overflow panic (rather than a clean Err) if corruption produced a max value
                    // of u64::MAX.
                    let max_row = treemap
                        .max()
                        .and_then(|m| m.checked_add(1))
        ...
        assert_eq!(
            checked,
            blob.len() * 8,
            "every single-bit flip must have been exercised"
        );
    }

    /// (6, inline-DV path) Same single-bit-flip sweep as above, but over the shorter, unframed
    /// inline payload (`deserialize_dv_bitmap` only, no `unframe_dv_blob`) -- the exact bytes an
    /// inline `DeltaSparkDvDescriptor.inline_data` carries. Bounded to one pass over one
    /// (shorter) payload's bits.
    #[test]
    fn inline_payload_single_bit_flip_sweep_never_panics() {
        let (_blob, data) = valid_dv_fixture();
        let mut checked = 0usize;
        ...
        for byte_idx in 0..data.len() {
            for bit in 0u8..8 {
                ...
                corrupted[byte_idx] ^= 1 << bit;
                let context = format!("inline byte {byte_idx} bit {bit} flipped");
                checked += 1;

                let decoded = assert_no_panic(&context, || deserialize_dv_bitmap(&corrupted));
                if let Ok(treemap) = decoded {
                    let max_row = treemap
                        .max()
                        .and_then(|m| m.checked_add(1))
        ...
        assert_eq!(
            checked,
            data.len() * 8,
            "every single-bit flip of the inline payload must have been exercised"
        );
    }
}
