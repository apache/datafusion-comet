...
/// Runtime tracking gate. The accounting allocator is always linked into the
/// default build, but balance tracking is skipped until a task turns it on (via
/// `arm` or `enable_tracking`), so an unused guard costs one relaxed load per
/// allocation. Stays on for the process lifetime once enabled.
...
/// `OomGuardPanic`, run `cleanup` (e.g. null a half-consumed stream/receiver so any
/// re-entry re-initializes) and return the mapped retriable error; otherwise re-raise
/// the original panic. Shared by the executePlan catch sites.
pub fn oom_error_or_resume(
    panic: Box<dyn std::any::Any + Send>,
    cleanup: impl FnOnce(),
) -> DataFusionError {
    match map_panic_to_error(panic.as_ref()) {
        Some(e) => {
            cleanup();
            e
        }
...
fn reset_for_test() {
    ...
    ARMED.store(false, Ordering::Relaxed);
    // Tests exercise the tracking path directly, so keep it on across resets.
    TRACKING_ENABLED.store(true, Ordering::Relaxed);
