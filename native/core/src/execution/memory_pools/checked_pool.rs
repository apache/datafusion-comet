...
/// A memory pool that refuses a reservation when the bytes the native allocator has actually
/// handed out, plus the request, would exceed a budget, and otherwise defers to an inner pool.
///
/// Every other pool only counts what operators voluntarily reserve, so native memory that bypasses
/// the pool is invisible to it until the executor exceeds its container limit. This pool consults
/// the `alloc-accounting` balance before the inner pool's own check, which turns that overrun into
/// a `ResourcesExhausted` error at the next reservation instead. The operators that can spill do
/// so; the ones that cannot fail the task.
///
...
/// off-heap allotment. So once any task pushes real usage to the budget, every task's next non-zero
/// reservation is denied. There is no per-task attribution, and allocations themselves are never
/// refused: this is a reservation gate, not a hard limit.
        ...
        if in_use.saturating_add(additional) > self.budget {
            return Err(resources_datafusion_err!(
                "Failed to reserve {additional} bytes for {}: native memory in use is {in_use} \
                 bytes of a {} byte budget. Reserved: {}",
