...
/// Number of live task-shared memory pools, which is the number of task attempts currently running
/// native plans. The real-usage fair-share guard uses this as the divisor for each task's share of
/// the executor budget. Incremented when a pool is constructed and decremented when it is dropped,
/// so it is exact even across the acquire/drop race that `TaskSharedMemoryPool::drop` handles.
static ACTIVE_TASK_COUNT: AtomicUsize = AtomicUsize::new(0);

/// Current number of concurrently active task-shared task attempts. Returns 0 when none are active
/// or the configured pool type keeps no task registry, in which case the guard falls back to a
/// fixed divisor.
...
impl Drop for TaskSharedMemoryPool {
    fn drop(&mut self) {
        // Paired with the increment in `acquire_task_shared_pool`. Both run exactly once per
        // pool, so the count never drifts even when the registry entry below belongs to a
        // replacement pool rather than this one.
