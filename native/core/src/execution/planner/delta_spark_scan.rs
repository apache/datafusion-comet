...
fn plan_delta_spark_scan(
        ...
        .any(|f| f.dv.is_some())
    {
        ...
        // get_partitioned_files maps 1:1 over the proto file list, so the
        // three sources zip index-aligned into one struct per file.
    ...
    let scan =
        planner.build_parquet_scan_plan(spark_plan.plan_id, common, object_store_url, files)?;
    Ok((vec![], vec![], scan))
}
...
mod tests {
    ...
    /// without touching any object-store backend: the key mirrors `parquet_support.rs`'s
    /// `url_key = scheme://{BeforeHost..AfterPort}` exactly, so these fixtures collide (or
    /// don't) under [`check_store_identity`] the same way the real closure's calls would.
    fn store_url_and_user_info(url_str: &str) -> (ObjectStoreUrl, String) {
        ...
        (ObjectStoreUrl::parse(key).unwrap(), url_user_info(&parsed))
    }
