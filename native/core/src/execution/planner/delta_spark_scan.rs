...
use crate::parquet::parquet_support::{
    hash_object_store_configs, object_store_registration_url, object_store_url_key,
...
fn plan_delta_spark_scan(
    ...
    let (object_store_url, object_store_backend, mut files) =
        ...
        .any(|f| f.dv.is_some())
    {
        ...
        // Resolve every store this partition touches here, on the JNI thread outside the
        // async DV runtime below: a cold S3 store's own internal block_on calls panic when
        // nested inside get_runtime().block_on(..). See attach_access_plans's doc comment.
        let mut resolver =
            PartitionStoreResolver::new(Arc::clone(&runtime_env), &object_store_options);
        ...
        let (mut store_references, mut memo_hits) = (0usize, 0usize);
        ...
        {
            let data = resolver.resolve(spark_file.file_path.clone())?;
            store_references += 1;
            memo_hits += usize::from(data.memo_hit);
            let data_store = data.store;
            ...
            {
                Some(dv_path) => {
                    let resolved = resolver.resolve(dv_path)?;
                    store_references += 1;
                    memo_hits += usize::from(resolved.memo_hit);
                    Some((resolved.store, resolved.path))
                }
                None => None,
            };
        ...
        // Most references in a partition share one store, so this stays near the reference count.
        log::debug!(
            "Delta partition resolved {store_references} store references with {memo_hits} memo hits"
        );
...
/// A store resolved by [`PartitionStoreResolver::resolve`]: the within-store `path` of the
/// URL, the `store` handle, and whether the resolver's memo already held that store.
...
/// Resolves the object store behind every data-file and deletion-vector URL one partition
/// touches (`check_same_object_store_authority` covers the data files; a DV may live under
/// another authority). Memoizes per registration URL so files sharing an authority pay the
/// global cache lock and `RuntimeEnv` registration once, and hosts the store-identity check.
struct PartitionStoreResolver<'a> {
    runtime_env: Arc<RuntimeEnv>,
    options: &'a HashMap<String, String>,
    /// `options` is the same map for every URL this partition resolves, so it is hashed once.
    config_hash: u64,
    resolved_stores: HashMap<ObjectStoreUrl, Arc<dyn ObjectStore>>,
    /// Per registration URL, the userinfo and raw URL of the first URL that resolved to it,
    /// for [`check_store_identity`].
    store_identities: HashMap<ObjectStoreUrl, (String, String)>,
}

impl<'a> PartitionStoreResolver<'a> {
    fn new(runtime_env: Arc<RuntimeEnv>, options: &'a HashMap<String, String>) -> Self {
        Self {
            runtime_env,
            options,
            config_hash: hash_object_store_configs(options),
            resolved_stores: HashMap::new(),
            store_identities: HashMap::new(),
        }
    }

    fn resolve(&mut self, url: String) -> Result<ResolvedStore, ExecutionError> {
        ...
        // The same normalize-then-key steps the shared resolution path applies, so the memo key
        // is the registration URL by construction. Re-parses only so the parse error above stays
        // redacted.
        let normalized = normalize_object_store_url(&url, self.options)?;
        let (url_key, _is_hdfs_scheme) = object_store_url_key(&normalized);
        let store_url = object_store_registration_url(&normalized, &url_key, self.config_hash)?;
        check_store_identity(&store_url, &user_info, &url, &mut self.store_identities)?;
        if let Some(store) = self.resolved_stores.get(&store_url) {
            let path = Path::from_url_path(normalized.url.path())
        ...
        // Memo miss: the expensive path (global cache lock, possible store construction,
        // runtime_env registration). It registers under `store_url`, which the drift guard in
        // parquet_support's tests pins, so the memo entry is read back under the same key.
        let (registered, path, _) = prepare_object_store_with_config_hash(
        ...
        debug_assert_eq!(
            registered, store_url,
            "memo key and registration URL must come from the same derivation"
        );
        let store = self.runtime_env.object_store(&store_url)?;
        self.resolved_stores.insert(store_url, Arc::clone(&store));
        Ok(ResolvedStore {
            path,
            store,
            memo_hit: false,
...
mod tests {
    ...
    /// Builds the same `(ObjectStoreUrl, userinfo)` pair `PartitionStoreResolver::resolve`
    /// computes for a URL, without touching any object-store backend: it runs the resolver's
    /// own normalize-then-key steps, so these fixtures collide (or don't) under
    /// [`check_store_identity`] the same way the real resolver's calls would.
    fn store_url_and_user_info(url_str: &str) -> (ObjectStoreUrl, String) {
        ...
        let (key, _) = object_store_url_key(&normalized);
        let store_url =
            object_store_registration_url(&normalized, &key, hash_object_store_configs(&configs))
                .unwrap();
        (store_url, user_info)
    }

    /// Resolves `first` then `second` (same authority) and asserts the second is a memo hit on
    /// the same store handle; `third`, when given, must be a miss that adds a second entry.
    fn assert_memoized_per_authority(
        options: &HashMap<String, String>,
        first: &str,
        second: &str,
        third: Option<&str>,
    ) {
        let mut resolver = PartitionStoreResolver::new(Arc::new(RuntimeEnv::default()), options);
        let a = resolver.resolve(first.to_string()).unwrap();
        assert!(!a.memo_hit, "{first} must miss a fresh memo");
        let b = resolver.resolve(second.to_string()).unwrap();
        assert!(b.memo_hit, "{second} must hit the memo entry {first} made");
        assert_eq!(resolver.resolved_stores.len(), 1);
        assert!(Arc::ptr_eq(&a.store, &b.store));
        assert_eq!(
            a.path,
            Path::from_url_path(Url::parse(first).unwrap().path()).unwrap()
        );
        assert_eq!(
            b.path,
            Path::from_url_path(Url::parse(second).unwrap().path()).unwrap()
        );
        if let Some(third) = third {
            let c = resolver.resolve(third.to_string()).unwrap();
            assert!(!c.memo_hit, "{third} must miss: different authority");
            assert_eq!(resolver.resolved_stores.len(), 2);
        }
    }

    #[test]
    #[cfg_attr(miri, ignore)] // AWS credential providers and object_store call foreign functions
    fn s3_files_sharing_a_bucket_hit_the_store_memo() {
        // The memo key must be the isolated registration URL the resolution path returns
        // (`s3+comet-<hash>-native://bucket`), not the physical `s3://bucket`; otherwise every
        // file after the first repeats the global cache lock and runtime registration.
        ...
        assert_memoized_per_authority(
            ...
            "s3://bucket/a.parquet",
            "s3://bucket/b.parquet",
            Some("s3://other/c.parquet"),
        );
    }

    /// A libhdfs-routed scheme keys its memo entry by the `hdfs` registration URL, so two
    /// files of one name node share the entry. The store is seeded in the process-wide cache,
    /// since the test build has no name node to construct one against.
    #[test]
    fn hdfs_files_sharing_a_name_node_hit_the_store_memo() {
        use crate::parquet::parquet_support::object_store_cache;
        use object_store::memory::InMemory;
        let options = HashMap::from([("fs.comet.libhdfs.schemes".to_string(), "hdfs".to_string())]);
        let cache_key = (
            "hdfs://comet-memo:8020".to_string(),
        ...
        object_store_cache()
            .write()
            .unwrap()
            .insert(cache_key.clone(), store);
        assert_memoized_per_authority(
            &options,
            "hdfs://comet-memo:8020/a.parquet",
            "hdfs://comet-memo:8020/b.parquet",
            None,
        );
        object_store_cache().write().unwrap().remove(&cache_key);
    }

    #[test]
    fn local_files_sharing_a_directory_hit_the_store_memo() {
        assert_memoized_per_authority(
            &HashMap::new(),
            "file:///tmp/x/a.parquet",
            "file:///tmp/x/b.parquet",
            None,
        );
    }
    ...
    fn dv_in_different_bucket_is_allowed() {
        ...
        // collision check were folded into it instead of PartitionStoreResolver::resolve.
        let mut seen = HashMap::new();
