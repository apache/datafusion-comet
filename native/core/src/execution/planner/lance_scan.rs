...
use comet_contrib_lance::{LanceScanConfig, LanceScanExec};
...
fn plan_lance_scan(spark_plan: &Operator, scan: &LanceScan) -> PlanCreationResult {
    ...
    let exec = LanceScanExec::try_new(LanceScanConfig {
        dataset_uri: common.dataset_uri.clone(),
        resolved_version: common.resolved_version,
        storage_options: common.storage_options.clone(),
        output_schema,
        filter_sql: common.filter_sql.clone(),
        limit: common.limit,
        offset: common.offset,
        batch_size: common.batch_size,
        spark_partition_index: partition.partition_index,
        fragment_ids: partition.fragment_ids.clone(),
    })
    ...
    Ok((
        vec![],
        vec![],
        Arc::new(SparkPlan::new(spark_plan.plan_id, Arc::new(exec), vec![])),
    ))
}
