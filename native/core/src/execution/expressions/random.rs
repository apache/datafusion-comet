...
impl ExpressionBuilder for RandStrBuilder {
    fn build(
        ...
        let expr = extract_expr!(spark_expr, RandStr);
        // The serde guarantees a non-negative length.
        let length = expr.length as usize;
        // Spark seeds a fresh generator per partition with `seed + partitionIndex`.
        let seed = expr.seed.wrapping_add(planner.partition().into());
        Ok(Arc::new(RandStrExpr::new(length, seed)))
    }
}
