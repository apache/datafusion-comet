...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("shuffle_writer: encode and compress (zstd)", |b| {
        let batch = create_batch(8192, true);
        let mut buffer = vec![];
        let mut cursor = Cursor::new(&mut buffer);
        let ipc_time = Time::default();
    ...
    group.bench_function("shuffle_writer: encode and compress (lz4 frame)", |b| {
        let batch = create_batch(8192, true);
        let mut buffer = vec![];
        let mut cursor = Cursor::new(&mut buffer);
        let ipc_time = Time::default();
    ...
    group.bench_function("shuffle_writer: encode and compress (lz4 block)", |b| {
        ...
        let ipc_time = Time::default();
        b.iter(|| {
            write_ipc_compressed(&batch, &mut cursor, &CompressionCodec::Lz4Block, &ipc_time)
        });
    });
