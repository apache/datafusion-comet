...
/*
 * This file was modified by the Apache DataFusion Comet project. See
 * native/hdfs-sys/README.md for the provenance of this directory and for the
 * full list of modifications. The change here is that getGlobalJNIEnv reports
 * whether it attached the current thread, and reuses an existing attachment
 * instead of calling AttachCurrentThread on a thread that already has one.
 */
    ...
    *attachedByLibhdfs = false;
	...
	if (hadoopJvmArgs != NULL)  {
        ...
        *attachedByLibhdfs = true;
        ...
        // and detaching it at thread exit. Comet attaches its own Tokio worker
        // threads, so that is the common case here rather than a corner one.
