...
use datafusion_comet_spark_expr::SparkHoursTransform;
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("hours_transform");
    for rows in ROW_COUNTS {
        for (null_ratio, tag) in NULL_RATIOS {
            let arr = timestamp_micros_array(rows, null_ratio, Some("UTC"), |i| {
                1_600_000_000_000_000 + (i as i64) * 3_600_000_000
            });
