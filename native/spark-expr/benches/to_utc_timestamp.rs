...
//! `to_utc_timestamp` is a datafusion-spark passthrough. Comet's serde wires it directly (and
//! `convert_timezone` is composed of `to_utc_timestamp` + `from_utc_timestamp`), so this bench
//! guards the pinned upstream kernel against regressions.
...
fn criterion_benchmark(c: &mut Criterion) {
    let udf = SparkToUtcTimestamp::new();
    ...
    let mut group = c.benchmark_group("to_utc_timestamp");
