...
pub fn spark_left_side_padding(args: &[ColumnarValue]) -> Result<ColumnarValue, DataFusionError> {
    spark_left_side_padding2(args, false)
}
...
pub fn spark_lpad(args: &[ColumnarValue]) -> Result<ColumnarValue, DataFusionError> {
    spark_left_side_padding2(args, true)
}

fn spark_left_side_padding2(
    args: &[ColumnarValue],
    truncate: bool,
    ...
    match args {
        [ColumnarValue::Array(array), ColumnarValue::Scalar(ScalarValue::Int32(Some(length)))] => {
            match array.data_type() {
                DataType::Utf8 => spark_left_side_padding_internal::<i32>(array, *length, truncate),
                DataType::LargeUtf8 => {
                    spark_left_side_padding_internal::<i64>(array, *length, truncate)
                }
                // Dictionary support required for SPARK-48498
                DataType::Dictionary(_, value_type) => {
                    let dict = as_dictionary_array::<Int32Type>(array);
                    let col = if value_type.as_ref() == &DataType::Utf8 {
                        spark_left_side_padding_internal::<i32>(dict.values(), *length, truncate)?
                    } else {
                        spark_left_side_padding_internal::<i64>(dict.values(), *length, truncate)?
                    };
...
fn spark_left_side_padding_internal<T: OffsetSizeTrait>(
    array: &ArrayRef,
    length: i32,
    truncate: bool,
) -> Result<ColumnarValue, DataFusionError> {
        ...
        match string {
            Some(string) => {
                ...
                if length <= char_len {
                    ...
                    padded.push_str(&space_string[char_len..]);
