...
object CometObjectHashAggregateExec
  ...
  /**
   * For Partial mode aggregates containing TypedImperativeAggregate functions (like CollectSet or
   * CollectList), the Spark-side output declares buffer columns as BinaryType (since Spark
   * serializes state to binary). However, the native Comet aggregate produces the actual state
   * type (e.g., ArrayType(elementType) for CollectSet/CollectList). This method corrects the
   * output schema to match the native state types so the shuffle exchange schema is consistent
