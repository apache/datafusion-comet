...
import org.apache.spark.sql.comet.execution.shuffle.NativeBatchDecoderIterator.getByteBufferAddress
...
case class NativeBatchDecoderIterator(
  ...
  def hasNext(): Boolean = {
    ...
    if (currentBatch != null) {
      println(s"Closing shuffle batch in JVM $currentBatch")
      currentBatch.close()
      currentBatch = null
    }
  ...
  private def fetchNext(): Option[ColumnarBatch] = {
    ...
    if (dataBuf.capacity() < bytesToRead) {
      ...
      println(s"Allocated new ByteBuffer ${getByteBufferAddress(dataBuf)}")
      threadLocalDataBuf.set(dataBuf)
    }
    println(s"BEGIN read into ByteBuffer ${getByteBufferAddress(dataBuf)}")
    dataBuf.clear()
    ...
    println(s"END read into ByteBuffer ${getByteBufferAddress(dataBuf)}")
    ...
    val batch = nativeUtil.getNextBatch(
      ...
      (arrayAddrs, schemaAddrs) => {
        ...
        println(s"call native decodeShuffleBlock for ByteBuffer ${getByteBufferAddress(dataBuf)}")
...
object NativeBatchDecoderIterator {
  private val threadLocalDataBuf: ThreadLocal[ByteBuffer] = ThreadLocal.withInitial(() => {
    ...
    println(s"DirectByteBuffer native address: ${getByteBufferAddress(buffer)}")
    buffer
  })

  def getByteBufferAddress(buffer: ByteBuffer): String = {
    val addressField = classOf[Buffer].getDeclaredField("address")
    addressField.setAccessible(true)
    "0x" + addressField.getLong(buffer).toHexString
  }
}
