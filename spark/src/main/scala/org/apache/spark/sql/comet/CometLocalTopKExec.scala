  ...
  override lazy val metrics: Map[String, SQLMetric] =
    CometMetricNode.baselineMetrics(sparkContext) ++ Map(
      "dynamic_filter_topk_rows_evaluated" ->
        SQLMetrics.createMetric(sparkContext, "Rows evaluated by the TopK runtime filter"),
      "dynamic_filter_topk_rows_pruned" ->
        SQLMetrics.createMetric(sparkContext, "Rows rejected before the TopK heap"),
      "dynamic_filter_topk_rows_bypassed" ->
        SQLMetrics.createMetric(sparkContext, "Rows bypassing an inactive TopK runtime filter"),
      "dynamic_filter_topk_eval_time" ->
        SQLMetrics.createNanoTimingMetric(
          sparkContext,
          "Time evaluating the TopK runtime filter"),
