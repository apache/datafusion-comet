...
object DeltaIntegration extends org.apache.spark.internal.Logging {
  ...
  // Change Data Feed support (`isCdfRelation` / `transformCdf` + their reflective binding) lands
  // with the CDF unit that also wires the `readChangeFeed` hook into CometExecRule -- shipping it
  // here would be dead code with no caller.
