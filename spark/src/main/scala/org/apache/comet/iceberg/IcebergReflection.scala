...
object IcebergReflection extends Logging {
  ...
  def rebuildDataFilesWithJavaMetrics(
    ...
    while (it.hasNext) {
      ...
      val footer = readParquetFooter(inputFile)
      val fieldMetrics = buildFloatFieldMetrics(dataFile, schema)
      val metrics = footerMetricsMethod.invoke(null, footer, fieldMetrics, metricsConfig, null)
   ...
   * NameMapping)`. In `iceberg-spark-runtime` jars parquet-mr is shaded, so the `ParquetMetadata`
   * parameter type is the relocated class -- resolved from the method itself rather than by name.
   */
  private lazy val footerMetricsMethod: java.lang.reflect.Method =
    loadClass("org.apache.iceberg.parquet.ParquetUtil").getMethods
  ...
  /**
   * Open `inputFile`'s parquet footer with the same (possibly shaded) parquet-mr classes
   * `footerMetrics` expects. `ParquetIO.file` bridges Iceberg's `InputFile` to parquet-mr's; it
   * is package-private, hence `setAccessible`.
   */
  private lazy val parquetIOFileMethod: java.lang.reflect.Method = {
    val method = loadClass("org.apache.iceberg.parquet.ParquetIO")
      .getDeclaredMethod("file", loadClass(ClassNames.INPUT_FILE))
    method.setAccessible(true)
    method
  }

  private lazy val parquetFileReaderOpenMethod: java.lang.reflect.Method = {
    val parquetMetadataClass = footerMetricsMethod.getParameterTypes()(0)
    val readerClass = loadClass(
      parquetMetadataClass.getName
        .replace("hadoop.metadata.ParquetMetadata", "hadoop.ParquetFileReader"))
    readerClass.getMethod("open", parquetIOFileMethod.getReturnType)
  }

  private def readParquetFooter(inputFile: AnyRef): AnyRef = {
    val parquetInputFile = parquetIOFileMethod.invoke(null, inputFile)
    val reader = parquetFileReaderOpenMethod.invoke(null, parquetInputFile)
    try {
