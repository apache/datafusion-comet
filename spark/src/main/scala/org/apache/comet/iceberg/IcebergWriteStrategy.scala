...
case class IcebergWriteStrategy(session: SparkSession) extends SparkStrategy {
  ...
  override def apply(plan: LogicalPlan): Seq[SparkPlan] = {
    ...
    plan match {
      ...
      // Hit by AQE.
      case l @ IcebergWriteLogical(child, batchWrite, write) =>
        Seq(IcebergWriteExec(batchWrite, write, l.output, planLater(child)))
      case _ => Nil
    }
  }
  ...
  private def buildTwoOp(
    ...
    Some(
      IcebergCommitExec(
        ...
        write,
        refresh,
        planLater(IcebergWriteLogical(query, batchWrite, write))))
  }
}
