...
object CometConf extends ShimCometConf {
  ...
  val COMET_SHUFFLE_RSS_MAX_IN_FLIGHT_BYTES: ConfigEntry[Long] =
    conf("spark.comet.shuffle.rss.maxInFlightBytes")
      .category(CATEGORY_SHUFFLE)
      .doc(
        "Maximum shuffle bytes admitted concurrently by all native Comet map attempts " +
          "sharing an executor-side Celeborn client. Admission covers native encoding " +
          "scratch and overlapping native, JNI, and Celeborn frame copies during submission; " +
          "accepted request bytes remain reserved until Celeborn reports completion.")
      .bytesConf(ByteUnit.BYTE)
      .checkValue(
