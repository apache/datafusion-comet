...
/**
 * Base type for [[org.apache.spark.sql.comet.CometRowViewWriteFilesExec]].
 *
 ...
 * class - so the row-view write is gated to Spark 4.0+ in `EliminateRedundantTransitions`. The
 * 3.x variant of this shim exists only to keep the shared sources compiling.
 */
