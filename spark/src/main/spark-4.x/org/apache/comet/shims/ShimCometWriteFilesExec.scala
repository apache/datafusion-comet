 ...
 * class - so this seam is gated to Spark 4.0+ in `CometExecRule` and native writes there go
 * through `CometDataWritingCommand` instead. The 3.x variant of this shim exists only to keep the
 * shared sources compiling.
 */
