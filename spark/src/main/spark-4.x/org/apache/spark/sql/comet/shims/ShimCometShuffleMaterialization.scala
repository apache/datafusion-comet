...
trait ShimCometShuffleMaterialization {
  ...
  // Spark 4 also associates artifacts with ordinary, non-Connect sessions.
  private val artifactState = JobArtifactSet.getCurrentJobArtifactState
    .orElse(activeSession.map(_.artifactManager.state))
    .orNull
