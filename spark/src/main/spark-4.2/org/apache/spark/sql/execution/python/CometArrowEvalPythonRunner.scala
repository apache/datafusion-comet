...
/**
 * Comet's Arrow Python runner for scalar Python UDFs on Spark 4.2. The Arrow IPC exchange lives
 * in [[CometArrowPythonRunnerBase]]; this subclass only supplies the Spark 4.2 constructor shape
...
class CometArrowEvalPythonRunner(
   ...
   *
   * Spark 4.2's worker does not read an input schema for `SQL_ARROW_BATCHED_UDF` (it derives the
   * types from the UDFs themselves), so nothing is written before the UDFs. Spark 4.1 is the one
