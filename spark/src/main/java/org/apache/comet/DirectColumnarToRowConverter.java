...
public final class DirectColumnarToRowConverter {
  ...
  public DirectColumnarToRowConverter(StructType schema) {
    ...
    long fixedSizeLong = nullBitsetWidth + (long) numFields * 8;
    if (fixedSizeLong > ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH) {
      throw new IllegalArgumentException("Schema is too wide for UnsafeRow conversion");
    }
    fixedSize = (int) fixedSizeLong;
    boolean fixed = true;
    for (int i = 0; i < numFields; i++) {
      ...
      switch (typeCodes[c]) {
        ...
        case FLOAT:
          if (isNull) {
            ...
            Platform.putLong(buffer, slot, 0L);
            Platform.putFloat(buffer, slot, col.getFloat(rowId));
          }
          break;
        case DOUBLE:
          if (isNull) {
            ...
            Platform.putDouble(buffer, slot, col.getDouble(rowId));
          }
          break;
        case STRING:
          if (isNull) {
  ...
  private void writeString(long slot, UTF8String value) {
    ...
    long roundedSize = ((long) numBytes + 7) & ~7L;
    ensureCapacity((long) cursor + roundedSize);
    if ((numBytes & 7) != 0) {
  ...
  private void convertBatchFixedWidth(int numRows) {
    ...
    if (totalSize > ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH) {
      ...
      case FLOAT:
        for (int r = 0; r < n; r++) {
          ...
          if (mayHaveNulls && col.isNullAt(r)) {
            ...
            Platform.putLong(
                batchBuffer, slot, Float.floatToRawIntBits(col.getFloat(r)) & 0xFFFFFFFFL);
          }
        }
        break;
      case DOUBLE:
        for (int r = 0; r < n; r++) {
          ...
          if (mayHaveNulls && col.isNullAt(r)) {
            ...
            Platform.putLong(batchBuffer, slot, Double.doubleToRawLongBits(col.getDouble(r)));
          }
        }
        break;
      case DECIMAL_COMPACT:
  ...
  private void ensureCapacity(long needed) {
    if (needed > ByteArrayMethods.MAX_ROUNDED_ARRAY_LENGTH) {
      throw new IllegalArgumentException("Row too large for UnsafeRow conversion");
    }
    if (needed > buffer.length) {
      int newSize =
          (int)
              Math.min(
                  Math.max(needed, (long) buffer.length * 2),
