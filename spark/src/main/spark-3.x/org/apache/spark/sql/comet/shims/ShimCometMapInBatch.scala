 ...
 *
 * Spark 3.5's `PythonArrowInput` trait has a different contract (`writeIteratorToArrowStream`
 * one-shot vs 4.x's `writeNextBatchToArrowStream` batch-at-a-time), so it cannot host the
 * columnar input implementation without a separate rewrite.
 */
