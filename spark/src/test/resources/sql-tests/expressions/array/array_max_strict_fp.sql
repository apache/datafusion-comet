...
statement
CREATE TABLE test_array_max_strict_fp(id int, d array<double>, f array<float>) USING parquet

statement
INSERT INTO test_array_max_strict_fp VALUES
...
query
SELECT id, array_max(d), array_max(f) FROM test_array_max_strict_fp
...
FROM test_array_max_strict_fp WHERE id IN (1, 2)
