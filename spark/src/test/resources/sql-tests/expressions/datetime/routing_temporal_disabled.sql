...
-- Config: spark.comet.expression.FromUTCTimestamp.allowIncompatible=false
-- Config: spark.comet.expression.ToUTCTimestamp.allowIncompatible=false
...
query expect_fallback(from_utc_timestamp: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT from_utc_timestamp(ts, 'UTC') FROM routing_temporal

query expect_fallback(to_utc_timestamp: spark.comet.exec.scalaUDF.codegen.enabled=false)
