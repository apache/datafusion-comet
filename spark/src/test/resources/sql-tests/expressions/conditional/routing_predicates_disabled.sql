...
query expect_fallback(equalto: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) = (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(equalnullsafe: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) <=> (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(lessthan: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) < (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(lessthanorequal: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) <= (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(greaterthan: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) > (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(greaterthanorequal: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) >= (t COLLATE UTF8_LCASE) FROM routing_predicates

query expect_fallback(contains: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT contains((s COLLATE UTF8_LCASE), (t COLLATE UTF8_LCASE)) FROM routing_predicates

query expect_fallback(startswith: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT startswith((s COLLATE UTF8_LCASE), (t COLLATE UTF8_LCASE)) FROM routing_predicates

query expect_fallback(endswith: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT endswith((s COLLATE UTF8_LCASE), (t COLLATE UTF8_LCASE)) FROM routing_predicates

query expect_fallback(in: spark.comet.exec.scalaUDF.codegen.enabled=false)
SELECT (s COLLATE UTF8_LCASE) IN ((t COLLATE UTF8_LCASE), 'other') FROM routing_predicates

query expect_fallback(inset: spark.comet.exec.scalaUDF.codegen.enabled=false)
