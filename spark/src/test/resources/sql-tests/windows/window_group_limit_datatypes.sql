...
-- which upstream Spark's window.sql does not exercise. All rank-like functions
-- (ROW_NUMBER, RANK, DENSE_RANK) route to Comet's streaming `PartitionedRankLimitExec`
-- and run natively.
--
-- Each ROW_NUMBER's ORDER BY carries `payload ASC` as a secondary key because `payload` is
-- unique per row; that removes tie-breaking non-determinism between engines even where
-- the streaming operator's input-order guarantee doesn't apply (e.g. plain ordering
-- comparisons in the outer query).
...
-- Decimal partition key with RANK.
query
...
-- Date partition key with DENSE_RANK.
query
