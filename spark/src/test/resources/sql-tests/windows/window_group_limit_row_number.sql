...
-- The `rn = 0` case at line 99 collapses to an empty LocalRelation under threshold=1000,
-- which lowers to a `LocalTableScan`; enable native execution for it so strict `query`
-- mode passes.
...
-- Limit = 0 collapses to an empty relation. Under threshold=1000, Spark's
-- InferWindowGroupLimit rewrites this to a `LocalTableScan` (Comet-eligible via the
-- fixture's `spark.comet.exec.localTableScan.enabled=true`); under threshold=-1 the
-- query runs via Window+Filter. Both plans return empty.
query
