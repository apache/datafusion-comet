...
-- All-null order column with RANK: every row shares rank 1.
query
SELECT a, b FROM (
...
-- All-null order column with DENSE_RANK: every row shares rank 1.
query
SELECT a, b FROM (
...
-- Every row tied — rank=1 returns all rows.
query
...
-- Every row tied — dense_rank=1 returns all rows.
query
SELECT count(*) AS cnt FROM (
