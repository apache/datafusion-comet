 ...
 * format per JVM in alternating order. comet requires the fused columnar reader; comet-row
 * disables vectorized cache reading to isolate the row iterator. Cache creation and validation
 * are outside timing.
 */
