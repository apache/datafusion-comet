...
abstract class CometTestBase
  ...
  def makeParquetFileAllPrimitiveTypes(
    ...
    data.foreach { opt =>
      ...
      opt match {
        case Some(i) =>
          ...
          record.add(8, Math.abs(i) % 256)
          record.add(9, Math.abs(i) % 65536)
          record.add(10, Math.abs(i))
          record.add(11, Math.abs(i.toLong))
    ...
    (0 until randomSize).foreach { _ =>
      ...
      // Use valid unsigned values (non-negative) for UINT fields
      record.add(8, Math.abs(i % 256).toInt)
      record.add(9, Math.abs(i % 65536).toInt)
      record.add(10, Math.abs(i).toInt)
      record.add(11, Math.abs(i))
      record.add(12, i.toString)
