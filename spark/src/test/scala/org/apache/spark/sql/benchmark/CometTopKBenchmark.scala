...
object CometTopKBenchmark extends CometBenchmarkBase {
  ...
  private def verifyAndReport(
    ...
    withSQLConf(mode.configs: _*) {
      ...
      // SQLMetric.value reports the residual evaluation time in nanoseconds.
      val topKMetricNames = Seq(
        "dynamic_filter_topk_filters_attached",
        "dynamic_filter_topk_filters_skipped",
        "dynamic_filter_topk_rows_evaluated",
        "dynamic_filter_topk_rows_pruned",
        "dynamic_filter_topk_rows_bypassed",
        "dynamic_filter_topk_eval_time")
      val topKMetrics = topKMetricNames.map { name =>
        name -> local.map(_.metrics(name).value).sum
      }.toMap
      require(
        topKMetrics("dynamic_filter_topk_filters_attached") ==
          (if (mode.filtering) partitions.toLong else 0L))
      if (mode.filtering) {
        require(
          topKMetrics("dynamic_filter_topk_rows_evaluated") +
            topKMetrics("dynamic_filter_topk_rows_bypassed") == scanMetrics("output_rows"))
      }
      val scanMetricNames = Seq(
        "bytes_scanned",
        "output_rows",
      ...
      val metrics = (topKMetricNames.map(name => s"$name=${topKMetrics(name)}") ++
        scanMetricNames.map(name => s"$name=${scanMetrics(name)}")).mkString(", ")
      benchmark.out.println(
        s"mode=${mode.name}, scan_partitions=$partitions, total_row_groups=$groups, $metrics")
    }
  }
