...
class CometFuzzTestSuite extends CometFuzzTestBase {
  ...
  ignore("select column with default value") {
  ...
  ignore("order by random columns") {
    val df = spark.read.parquet(filename)
    df.createOrReplaceTempView("t1")
  ...
  ignore("distribute by single column (complex types)") {
    val df = spark.read.parquet(filename)
    df.createOrReplaceTempView("t1")
    val columns = df.schema.fields.filter(f => isComplexType(f.dataType)).map(_.name)
    for (col <- columns) {
  ...
  ignore("shuffle supports all types") {
    val df = spark.read.parquet(filename)
