  ...
  test("select temporary column") {
    ...
    for (col <- df.columns
      ...
      withTable("t2") {
        ...
        // Verify that our default value matches Spark and that it matches what we originally selected out of t1.
        val sql = "select col2 from t2"
        // TODO: This fails in Spark's vectorized Parquet reader for DECIMAL(36,18) or BINARY column types.
        if (default_value_type != "DECIMAL(36,18)" && default_value_type != "BINARY") {
          if (CometConf.isExperimentalNativeScan) {
            checkSparkAnswerAndOperator(sql)
