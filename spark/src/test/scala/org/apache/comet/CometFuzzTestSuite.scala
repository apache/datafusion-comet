...
class CometFuzzTestSuite extends CometFuzzTestBase {
  ...
  test("select column with default value") {
    ...
    withSQLConf(SQLConf.PARQUET_VECTORIZED_READER_ENABLED.key -> "false") {
      // This test relies on two tables: 1) t1 the shallow Parquet fixture with random values, and
      ...
      for (col <- columns) {
        ...
        withTable("t2") {
          ...
          if (defaultValueType == "TIMESTAMP_NTZ") {
            // This table deliberately requests a top-level NTZ field from Parquet, unlike
            // the wrapped source fixture. Its schema cannot prove the physical storage type.
