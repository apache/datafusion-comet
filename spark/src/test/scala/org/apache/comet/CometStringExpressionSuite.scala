...
class CometStringExpressionSuite extends CometTestBase {
  ...
  test("regexp_extract basic") {
    ...
    withSQLConf(CometConf.COMET_REGEXP_ALLOW_INCOMPATIBLE.key -> "true") {
      ...
      withParquetTable(data, "tbl") {
        ...
        // Test group 2
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract(_1, '(\\d+)-(\\d+)', 2) FROM tbl")
        // Test non-existent group → should return ""
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract(_1, '(\\d+)-(\\d+)', 3) FROM tbl")
      }
    }
  }
  ...
  test("regexp_extract edge cases") {
    ...
    withSQLConf(CometConf.COMET_REGEXP_ALLOW_INCOMPATIBLE.key -> "true") {
      ...
      withParquetTable(data, "tbl") {
        ...
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract(_1, '@([^.]+)', 1) FROM tbl")
  ...
  test("regexp_extract_all basic") {
    ...
    withSQLConf(CometConf.COMET_REGEXP_ALLOW_INCOMPATIBLE.key -> "true") {
      ...
      withParquetTable(data, "tbl") {
        // Test default (group 0)
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract_all(_1, '\\d+') FROM tbl")
        // Test with explicit group 0
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract_all(_1, '(\\d+)', 0) FROM tbl")
        // Test group 1
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract_all(_1, '(\\d+)', 1) FROM tbl")
      }
    }
  }
  ...
  test("regexp_extract_all multiple matches") {
    ...
    withSQLConf(CometConf.COMET_REGEXP_ALLOW_INCOMPATIBLE.key -> "true") {
      ...
      withParquetTable(data, "tbl") {
        ...
        // Extract all words
        checkSparkAnswerAndOperator(
          "SELECT regexp_extract_all(_1, '([a-z]+)', 1) FROM tbl")
      }
    }
  }
