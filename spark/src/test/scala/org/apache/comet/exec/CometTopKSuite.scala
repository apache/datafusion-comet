...
class CometTopKSuite extends CometTestBase {
          ...
          withParquetTable(path.toString, "topk_groups") {
            ...
            Seq(false, true).foreach { enabled =>
              withSQLConf(
                ...
                val evaluated = local.metrics("dynamic_filter_topk_rows_evaluated").value
                val pruned = local.metrics("dynamic_filter_topk_rows_pruned").value
                val bypassed = local.metrics("dynamic_filter_topk_rows_bypassed").value
                val evalTime = local.metrics("dynamic_filter_topk_eval_time").value
                if (enabled) {
                  // Every decoded row reaches the residual filter exactly once. Reader pruning
                  // reduces this total; residual pruning reduces the rows passed to TopK.
                  assert(evaluated + bypassed == metrics("output_rows"))
                  assert(pruned <= evaluated)
                  assert(metrics("output_rows") - pruned >= local.metrics("output_rows").value)
                  assert(evaluated > 0L && bypassed > 0L && evalTime > 0L)
                  if (!pageIndex && !rowFilter) {
                    assert(pruned > 0L)
                  }
                } else {
                  assert(evaluated == 0L && pruned == 0L && bypassed == 0L && evalTime == 0L)
                }
    ...
    withParquetTable((0 until 100).map(i => Tuple1(i)), "topk_input") {
      for (enabled <- Seq(false, true)) {
        withSQLConf(CometConf.COMET_EXEC_TOPK_DYNAMIC_FILTER_ENABLED.key -> enabled.toString) {
          ...
          if (enabled) {
            assert(local.metrics("dynamic_filter_topk_filters_attached").value > 0L)
          } else {
            assert(local.metrics("dynamic_filter_topk_filters_attached").value == 0L)
          }
          assert(
  ...
  for ((requiredType, oldType) <- Seq(("INT", "int64"), ("BIGINT", "int32"))) {
    test(s"TopK preserves mixed-file schema conversion errors: $requiredType") {
      def isSchemaError(error: Throwable): Boolean =
        Iterator
          .iterate(error)(_.getCause)
          .takeWhile(_ != null)
          .exists(_.isInstanceOf[
      ...
      withSQLConf(
        ...
        withTempDir { dir =>
          val cleanType = if (requiredType == "INT") "int32" else "int64"
          for ((name, physicalType, values) <- Seq(
              ("clean", cleanType, 0 until 1000),
              ("older", oldType, 10000 until 10100))) {
            val path = new org.apache.hadoop.fs.Path(dir.toURI.toString, s"$name.parquet")
            val schema = org.apache.parquet.schema.MessageTypeParser.parseMessageType(
              s"message root { required int32 k; required $physicalType payload; }")
            val writer = createParquetWriter(schema, path, rowGroupSize = 1L)
            try {
              values.foreach { key =>
                val row = new org.apache.parquet.example.data.simple.SimpleGroup(schema)
                row.add(0, key)
                if (physicalType == "int64") row.add(1, key.toLong) else row.add(1, key)
                writer.write(row)
              }
            } finally {
              writer.close()
          ...
          withTempView("topk_schema") {
            spark.read
              .schema(s"k INT, payload $requiredType")
              .parquet(dir.getCanonicalPath)
              .createOrReplaceTempView("topk_schema")
            for (enabled <- Seq(false, true)) {
              withSQLConf(
                CometConf.COMET_EXEC_TOPK_DYNAMIC_FILTER_ENABLED.key -> enabled.toString) {
                val query = sql("SELECT k, payload FROM topk_schema ORDER BY k LIMIT 10")
                ...
                if (requiredType == "INT" || org.apache.spark.SPARK_VERSION.startsWith("3.")) {
                  assert(sparkError.exists(isSchemaError), s"Spark: $sparkError")
                  assert(cometError.exists(isSchemaError), s"filtering=$enabled: $cometError")
                } else {
                  assert(sparkError.isEmpty && cometError.isEmpty)
                  checkAnswer(query, (0 until 10).map(key => Row(key, key.toLong)))
                }
              }
            }
          }
