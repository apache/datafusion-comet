...
class CometExecSuite extends CometTestBase {
  ...
  // Repro for the Spark 3.5 SQL-tests failure on subquery/exists-subquery/exists-orderby-limit.sql
  // (query #19) on branch opt_native_shuffle. Crashes with NativeUtil.exportBatch:132 AIOOBE
  // on this branch but passes on main. The SQL file declares three CONFIG_DIM1 combos for
  // codegen; the failure trace doesn't say which one fires it, so sweep all three.
  test("EXISTS subquery with GROUP BY + LIMIT + OFFSET") {
    withTempView("emp", "dept") {
      spark
        .sql("""SELECT * FROM VALUES
               |  (100, 'emp 1', date '2005-01-01', 100.0D, 10),
               |  (200, 'emp 2', date '2003-01-01', 200.0D, 10),
               |  (300, 'emp 3', date '2002-01-01', 300.0D, 20),
               |  (400, 'emp 4', date '2005-01-01', 400.0D, 30),
               |  (500, 'emp 5', date '2001-01-01', 400.0D, NULL),
               |  (700, 'emp 7', date '2010-01-01', 400.0D, 100),
               |  (800, 'emp 8', date '2016-01-01', 150.0D, 70)
               |AS t(id, emp_name, hiredate, salary, dept_id)""".stripMargin)
      ...
      spark
        .sql("""SELECT * FROM VALUES
               ...
               |  (70, 'dept 7', 'FL')
               |AS t(dept_id, dept_name, state)""".stripMargin)
        .createOrReplaceTempView("dept")

      val configDims = Seq(
        Map("spark.sql.codegen.wholeStage" -> "true"),
        Map(
      ...
      // Mirror the SQL test order: queries #1 through #16 from the file run before #17 (the
      // one whose CI failure we're chasing). Query #11's subquery is identical except for
      // the OFFSET; #13 and #15 share similar shapes. Subquery materialization / AQE plan
      // cache state from running them first may alter query #17's executed shape.
      val priorQueries = Seq(
        // #1
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state) ORDER BY hiredate""",
        // #2
        """SELECT id, hiredate FROM emp WHERE EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state) ORDER BY hiredate DESC""",
        // #3
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state LIMIT 1) ORDER BY hiredate""",
        // #4
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state LIMIT 0) ORDER BY hiredate""",
        // #5
        """SELECT * FROM emp WHERE NOT EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state) ORDER BY hiredate""",
        // #6
        """SELECT emp_name FROM emp WHERE NOT EXISTS (SELECT max(dept.dept_id) a FROM dept WHERE dept.dept_id = emp.dept_id GROUP BY state ORDER BY state)""",
        // #7
        """SELECT count(*) FROM emp WHERE NOT EXISTS (SELECT max(dept.dept_id) a FROM dept WHERE dept.dept_id = emp.dept_id GROUP BY dept_id ORDER BY dept_id)""",
        // #8
        """SELECT * FROM emp WHERE NOT EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state LIMIT 1) ORDER BY hiredate""",
        // #9
        """SELECT * FROM emp WHERE NOT EXISTS (SELECT dept.dept_id FROM dept WHERE emp.dept_id = dept.dept_id ORDER BY state LIMIT 0) ORDER BY hiredate""",
        // #10
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_name FROM dept WHERE dept.dept_id > 10 LIMIT 1)""",
        // #11 - same subquery as #17 minus the OFFSET
        """SELECT * FROM emp WHERE EXISTS (SELECT max(dept.dept_id) FROM dept GROUP BY state LIMIT 1)""",
        // #12
        """SELECT * FROM emp WHERE NOT EXISTS (SELECT dept.dept_name FROM dept WHERE dept.dept_id > 100 LIMIT 1)""",
        // #13
        """SELECT * FROM emp WHERE NOT EXISTS (SELECT max(dept.dept_id) FROM dept WHERE dept.dept_id > 100 GROUP BY state LIMIT 1)""",
        // #14
        """SELECT emp_name FROM emp WHERE NOT EXISTS (SELECT max(dept.dept_id) a FROM dept WHERE dept.dept_id = emp.dept_id GROUP BY state ORDER BY state LIMIT 2 OFFSET 1)""",
        // #15
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_name FROM dept WHERE dept.dept_id > 10 LIMIT 1 OFFSET 2)""",
        // #16
        """SELECT * FROM emp WHERE EXISTS (SELECT dept.dept_name FROM dept WHERE dept.dept_id > emp.dept_id LIMIT 1)""")

      for (dim <- configDims) {
        // SQL-tests harness (dev/diffs/3.5.8.diff, 4.1.1.diff) sets only:
        //   spark.comet.enabled, spark.comet.exec.enabled, spark.comet.exec.shuffle.enabled,
        //   spark.comet.parquet.respectFilterPushdown, spark.shuffle.manager,
        //   spark.comet.memoryOverhead.
        // It does NOT enable spark.comet.sparkToColumnar.enabled, but CometTestBase does.
        // Force the harness shape so the reproducer matches the failing config.
        val harnessConf = dim ++ Map(CometConf.COMET_SPARK_TO_ARROW_ENABLED.key -> "false")
        withSQLConf(harnessConf.toSeq: _*) {
          // scalastyle:off println
          println(s"\n===== config dim: $harnessConf =====")
          for ((q, idx) <- priorQueries.zipWithIndex) {
            spark.sql(q).collect()
            println(s"--- ran prior query #${idx + 1} ---")
          }
          val sql = """SELECT *
                      |FROM emp
          ...
          println("--- query #17 initial executedPlan ---")
          println(df.queryExecution.executedPlan)
          val rows = df.collect()
          println("--- query #17 final (post-AQE) executedPlan ---")
          println(df.queryExecution.executedPlan)
          println(s"--- ${rows.length} rows ---")
