...
class CometDeltaNativeSuite extends CometTestBase {
  ...
  private def deltaAvailable: Boolean = {
    try {
      Class.forName("org.apache.spark.sql.delta.DeltaLog")
      true
    } catch {
   ...
   */
  private def checkNativeScan(query: String): Unit = {
    val (_, cometPlan) = checkSparkAnswer(query)
    val nativeScans = collectNativeScans(cometPlan)
    assert(
  ...
  test("create and query simple Delta table") {
    assume(deltaAvailable, "Delta not available in classpath")

    withTempDeltaDir { warehouseDir =>
      spark.sql(s"""
        CREATE TABLE test_table (
          id INT,
          name STRING,
          value DOUBLE
        ) USING delta LOCATION '${warehouseDir.toString()}'
      """)
      ...
      checkNativeScan("SELECT * FROM test_table ORDER BY id")
  ...
  def withTempDeltaDir(f: File => Unit): Unit = {
    val dir = Files.createTempDirectory("comet-delta-test").toFile
    try {
      f(dir)
    } finally {
