...
#
# Options:
#   --spark-version <version>  Only regenerate for specified Spark version (3.5, 4.0, 4.1, or 4.2)
...
# queries whose plans diverge from the previous tier. The base (3.5) profile has
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            -h|--help)
                ...
                echo ""
                echo "Options:"
                echo "  --spark-version <version>  Only regenerate for specified Spark version (3.5, 4.0, 4.1, or 4.2)"
    ...
    if [ -n "$target_version" ]; then
        if [[ ! "$target_version" =~ ^(3\.5|4\.0|4\.1|4\.2)$ ]]; then
            echo "[ERROR] Invalid Spark version: $target_version"
            echo "[ERROR] Supported versions: 3.5, 4.0, 4.1, 4.2"
            exit 1
        fi
    fi
