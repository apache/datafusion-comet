...
description: Use when reviewing a DataFusion Comet pull request. Covers the workflow that applies to every PR and routes to the area-specific review skills for expressions, FFI, memory management, the scan, and shuffle. Provides guidance to a human reviewer rather than posting comments.
...
| `CometExecIterator`, `CometNativeArrowSource`, `NativeUtil`, `CometVector` subclasses, `jni_api.rs`, `execution/operators/scan.rs`, `aligned_stream_reader.rs`, anything using `FFI_Arrow*` | `review-comet-ffi-pr`        |
...
| `CometScanRule`, `CometScanContrib`, `CometScanExec`, `CometNativeScanExec`, `CometBatchScanExec`, `CometNativeScan.scala`, `native/core/src/parquet/`                                      | `review-comet-scan-pr`       |
