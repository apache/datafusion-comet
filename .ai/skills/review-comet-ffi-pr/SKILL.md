...
whose listener charges them to Spark, so a new path that exports them without going through
`CometArrowStream`, which moves the charge off the task, charges them on both sides. Exported
native batches are usually no longer pool-reserved by the time the JVM receives them, but they stay
resident until the JVM closes them.
